...
import type { EvChargePowerResolution, EvEnergyNeed } from "./types";

export const EV_AC_VOLTAGE_V = 230;
const KWH_DECIMALS = 3;
const MIN_DECIMALS = 1;

export function roundKwh(n: number): number {
	return Math.round(n * 10 ** KWH_DECIMALS) / 10 ** KWH_DECIMALS;
}

export function roundMinutes(n: number): number {
...
export function energyForSocDeltaKwh(input: {
	...
	if (batteryCapacityKWh <= 0 || chargingEfficiency <= 0) return null;
...
export function evccDerivedChargePowerKw(input: {
...
}): { powerKw: number | null; currentAUsed: number | null; phasesUsed: number | null } {
	const currentAUsed =
		positive(input.effectiveMaxCurrentA) ??
		positive(input.maxCurrentA) ??
		positive(input.offeredCurrentA);
	const phasesUsed =
		positive(input.phasesConfigured) ??
	...
	return {
		powerKw: roundKwh((currentAUsed * phasesUsed * EV_AC_VOLTAGE_V) / 1000),
		currentAUsed,
		phasesUsed,
	};
}
 ...
 */
export function resolveChargePower(model: EvModelV1): EvChargePowerResolution {
...
export function computeEnergyNeed(
	model: EvModelV1,
	...
	const minutesFrom =
		model.minimumDepartureSocPct != null ? energyToDepartureMinimumKWh : energyToTargetKWh;
	return {
		energyToTargetKWh,
...
export function chargingMinutesForEnergy(
	energyKWh: number | null,
	chargePowerKw: number | null,
): number | null {
	if (energyKWh == null || chargePowerKw == null || chargePowerKw <= 0) return null;
