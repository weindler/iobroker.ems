...
**Status:** Plan only — no object deletion or JSON relocation in 4A.  
**Baseline:** tag `planner-shadow-gate3-verified`, version 0.1.143.
...
### 4B — Stop over-creation (no deletes)

- Ensure AC unit/mapping states only when unit enabled.  
- Ensure vehicle profile states only for configured `wb_vehicle_profiles`.  
- Skip takeover/authority ensure when `planner_runtime_mode=off` (already partially lazy).  
- **Expected reduction:** hundreds of unused leaves on typical installs.
...
### 4E — JSON → files (after reader audit)
