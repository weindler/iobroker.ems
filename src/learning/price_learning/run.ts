...
export async function runPriceLearning(
  host: PriceLearningRunHost,
): Promise<void> {
  ...
  try {
    host.log.debug?.(
      `Price Learning: loading history (${cfg.lookbackDays}d, ${cfg.priceStateId.split(".").slice(-2).join(".")})…`,
    );
    const { samples } = await fetchPriceSamples(
      host,
      cfg.priceStateId,
      cfg.lookbackDays,
    );
    ...
    host.log.debug?.(
      `Price Learning completed: status=${result.status} confidence=${result.confidence}`,
    );
