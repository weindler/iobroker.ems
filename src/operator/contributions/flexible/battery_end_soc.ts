...
export type DynamicBatteryEndSocInput = {
	capacityKwh: number;
	socPct: number;
	minSocPct: number;
	maxSocPct: number;
	modePolicy: PlannerModePolicy;
	/** Gelerntes Nachtentladen; null = unbekannt. */
	avgNightDischargeKwh: number | null;
	/** Ergebnis der PV-Defizit-/Recovery-Bilanz (auch wenn active=false). */
	chargeLogic: BatteryChargeLogicDecision | null;
	/**
	 * Eco: späterer Netzstrom klar günstiger → nicht-defizitäres Ziel auf Nacht-/Safety-Floor
	 * drücken (kein Fake-100 %).
	 */
	deferForCheapFutureGrid?: boolean;
};

export type DynamicBatteryEndSocResult = {
	socTargetPct: number;
	energyTargetKwh: number;
	/** true wenn nur Policy-Fallback (keine Night-/Horizont-Daten). */
...
function nightFactor(mode: PlannerModePolicy["mode"]): number {
	if (mode === "eco") return 1.0;
	if (mode === "comfort" || mode === "forced") return 1.25;
	return 1.1;
}

function morningCushionFactor(mode: PlannerModePolicy["mode"]): number {
	if (mode === "eco") return 0.15;
	if (mode === "comfort" || mode === "forced") return 0.45;
	return 0.3;
}
 ...
 */
export function planDynamicBatteryEndSoc(input: DynamicBatteryEndSocInput): DynamicBatteryEndSocResult {
	...
	const nightRaw =
		input.avgNightDischargeKwh != null &&
	...
	const deficit =
		logic?.energyDeficitKwh != null && logic.energyDeficitKwh > 0 ? logic.energyDeficitKwh : 0;
	...
	if (hasHorizon && deficit > 0 && logic?.energyTargetKwh != null) {
		...
		parts.push(
			`PV-Defizit bis Recovery ${logic.pvRecoveryDay ?? "?"} → Ziel ${logic.energyTargetKwh.toFixed(1)} kWh`,
		);
	} else if (hasHorizon && deficit <= 0) {
		parts.push(
			logic?.pvRecoveryDay != null
