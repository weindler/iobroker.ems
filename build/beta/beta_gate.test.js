...
(0, node_test_1.describe)("BETA-GATE-008 restore dryrun clamp", () => {
    ...
    (0, node_test_1.it)("restore barrier blocks device writes even when global live", async () => {
        const { setRestoreInProgress, resetRestoreBarrierForTest } = await Promise.resolve().then(() => __importStar(require("../restore/barrier.js")));
        const { writeForeignIfChanged } = await Promise.resolve().then(() => __importStar(require("../device_write.js")));
