...
export type ThermalSoftUrgency = {
	/** 0..1 Speichernutzen für Soft-Score (Overnight-Lücke hoch, mehrtägige Reichweite ~0). */
	needScale: number;
	remainingHours: number | null;
	/** Bekannt mehrtägig voll — schwache/späte Soft-Fenster weglassen. */
	skipWeakSoftWindows: boolean;
	/** Lieber zusammenhängende Blöcke statt Mindeststufen-Häppchen. */
	requireCoherentBlock: boolean;
};

const LONG_RANGE_H = 36;
const COHERENT_RANGE_H = 24;

export function resolveThermalSoftUrgency(input: {
	nowMs: number;
	emptyMs: number | null;
