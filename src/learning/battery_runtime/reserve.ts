...
export const NIGHT_RESERVE_SAFETY_MARGIN_FRACTION = 0.2;

export type RequiredSocAtPvEndInput = {
	predictedNightConsumptionKwh: number | null;
	usableCapacityKwh: number | null;
	safetyMarginFraction?: number;
};

export type RequiredSocAtPvEndResult = {
...
export function resolveRequiredSocAtPvEndPct(
	input: RequiredSocAtPvEndInput,
): RequiredSocAtPvEndResult {
	const margin = input.safetyMarginFraction ?? NIGHT_RESERVE_SAFETY_MARGIN_FRACTION;
