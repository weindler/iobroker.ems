...
describe("planner_authorization lazy load", () => {
	it("authorization_mode parse alone does not load service", () => {
		const modules = run(`
...
`);
	...
	it("action_bridge alone does not load service or audit_io", () => {
		const modules = run(`
const path = require("path");
require(path.join(process.cwd(), "build/planner_authorization/action_bridge.js"));
console.log(Object.keys(require.cache).join("\\n"));
`);
		assert.ok(!modules.some((e) => e.includes("/build/planner_authorization/service.js")));
		assert.ok(!modules.some((e) => e.includes("/build/planner_authorization/audit_io.js")));
	});
});
