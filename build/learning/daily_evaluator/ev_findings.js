...
exports.evaluateEvFindings = void 0;
...
function evaluateEvFindings(day) {
    const findings = [];
    ...
    for (const s of day.forecastSnapshots) {
        ...
        if (target == null || !s.wallboxDeadlineIso)
            continue;
    ...
    for (const [deadlineIso, snap] of byDeadline) {
        ...
        if (!Number.isFinite(deadlineMs) || deadlineMs < day.startMs || deadlineMs >= day.endMs)
            continue;
        const idx = (0, slots_1.slotIndexForMs)(layout, deadlineMs) ?? layout.slotCount - 1;
        let observedSoc = null;
        for (let i = idx; i >= 0; i--) {
...
exports.evaluateEvFindings = evaluateEvFindings;
