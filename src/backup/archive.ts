import { assertUniqueArchivePaths } from "./manifest_validate";
import type { ExportArchiveEntry } from "./types";
 ...
 */
export function buildZipArchive(entries: ExportArchiveEntry[]): Buffer {
...
export function readZipEntryNames(archive: Buffer): string[] {
	const names: string[] = [];
	let pos = 0;
...
export function readZipEntryData(archive: Buffer, entryName: string): Buffer | null {
	let pos = 0;
