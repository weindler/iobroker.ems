...
(0, node_test_1.describe)("JOINT-E3 live surplus: Soft jetzt, nicht erst Wochenende", () => {
    (0, node_test_1.it)("preferLive + SOC≥90 + Soft-Headroom → IH im heutigen NOW-Fenster", () => {
        const nowIso = "2026-08-21T07:30:00.000Z"; // 09:30 CEST Freitag
        const slots = (0, fixtures_1.buildSlots)(nowIso, 96); // bis Sonntag
        const input = scenarioAInput();
        input.time = {
            ...input.time,
            nowIso,
        ...
        input.pv = {
            ...
            slots: slots.map((s) => {
                ...
                const h = new Date(s.startIso).getUTCHours();
                let power = 0;
                if (day === "2026-08-21" && h >= 7 && h < 16)
                    power = 4500;
                else if (day === "2026-08-22" && h >= 10 && h < 16)
                    power = 5500; // Samstag stärker
                else if (day === "2026-08-23" && h >= 10 && h < 16)
                    power = 5500;
                return {
                    slot: s,
                    forecastPowerW: power,
        ...
        const ihToday = plan.allocations
            ...
            .reduce((s, a) => s + a.allocatedEnergyKwh, 0);
        strict_1.default.ok(ihToday > 0.3, `Soft bei Live-Surplus heute, got heute=${ihToday}`);
        const batIh = sumKind(plan, "immersion_heater", (a) => a.energySource === "battery" || a.energySource === "mixed");
        strict_1.default.equal(batIh, 0);
    });
});
