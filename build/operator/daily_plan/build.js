...
exports.buildAllocationCandidate = exports.dailyPlanRevisionPayload = exports.buildDailyPlanFromForecast = exports.buildDailyPlan = void 0;
...
function accumulateRequestedEnergy(byContribution, contributionId, requestedEnergyKwh) {
    if (requestedEnergyKwh === null || !Number.isFinite(requestedEnergyKwh))
        return;
    const prev = byContribution.get(contributionId);
    byContribution.set(contributionId, prev === undefined ? requestedEnergyKwh : Math.max(prev, requestedEnergyKwh));
}
function sumRequestedByContribution(byContribution) {
    let sum = 0;
    for (const v of byContribution.values())
        sum += v;
    return sum;
}
function computeTotals(slots, allocations, day) {
    let pvAllocated = 0;
    let gridAllocated = 0;
    let gridCost = 0;
    let hasCost = false;
    ...
    let flexReq = 0;
    let flexAlloc = 0;
    const flexReqByContribution = new Map();
    const mandatoryReqByContribution = new Map();
    for (const a of allocations) {
        const e = a.allocatedEnergyKwh ?? 0;
        if (a.pvPowerW > 0)
            pvAllocated += (0, slots_1.energyKwhFromPower)(a.pvPowerW, SLOT_MINUTES);
        if (a.gridPowerW > 0)
            gridAllocated += (0, slots_1.energyKwhFromPower)(a.gridPowerW, SLOT_MINUTES);
        if (a.estimatedCostCt !== null) {
            gridCost += a.estimatedCostCt;
            hasCost = true;
        }
        if (a.contributionId === contribution_ids_1.CONTRIBUTION_IDS.BATTERY_CHARGE)
            battery += e;
        else if (a.contributionId === contribution_ids_1.CONTRIBUTION_IDS.WALLBOX_EV_SESSION)
            wallbox += e;
        else if (a.contributionId.startsWith("immersion_heater"))
            immersion += e;
        else if (a.contributionId.startsWith("air_conditioning"))
            ac += e;
        if (a.mandatory) {
            mandatoryAlloc += e;
    ...
    return {
        ...
        mandatoryUnallocatedEnergyKwh: mandatoryReq > 0 ? round3(Math.max(0, mandatoryReq - mandatoryAlloc)) : null,
    };
}
function resolveDailyPlanStatus(forecastStatus, timezone, hasSlots, degraded) {
    if (!timezone.trim())
        return "error";
    if (forecastStatus === "missing_inputs")
...
function partitionExcluded(candidates) {
    return candidates
...
function buildDailyPlan(input) {
    ...
    const hasMandatoryGap = allocationResult.unallocated.some((u) => u.unallocatedEnergyKwh !== null &&
        u.unallocatedEnergyKwh > 0 &&
        candidates.find((c) => c.contributionId === u.contributionId)?.mandatory);
    const hasDegradedContributions = input.contributions.some((c) => c.enabled && c.quality.status === "degraded");
...
exports.buildDailyPlan = buildDailyPlan;
function buildDailyPlanFromForecast(now, timezone, globalMode, forecastPlan, policy) {
