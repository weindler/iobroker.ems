...
function observedSelfConsumption(day) {
    let energy = 0;
    let basis = 0;
    let pairs = 0;
...
function observedAutonomy(day) {
    ...
    for (let index = 0; index < day.slotCount; index++) {
        ...
        if (finite(house) !== finite(imported))
            unpaired = true;
        if (!finite(house) || !finite(imported))
            continue;
 ...
 */
function fastChargeAttribution(day) {
    ...
    for (let index = 0; index < day.slotCount; index++) {
        ...
        if (finite(ev) && ev > 0 && !finite(fastKwh))
            modeComplete = false;
        if (!finite(fastKwh))
            continue;
...
exports.fastChargeAttribution = fastChargeAttribution;
/**
 ...
 */
function attributeDevicePv(day, device) {
...
function batteryPvAttribution(day) {
...
function batteryCapacityKwh(day) {
    ...
    if (min <= 0 || max - min > Math.max(0.1, max * 0.02))
        return null;
    return values[values.length - 1] ?? null;
}
function batteryDomainComplete(day) {
    const masks = day.buckets.qualityMask.filter(finite);
    if (masks.length === 0)
        return false;
    return masks.every((mask) => (0, quality_mask_1.decodeDomainQuality)(mask, quality_mask_1.TELEMETRY_DOMAIN.BATTERY) === quality_mask_1.DOMAIN_QUALITY.ok);
}
function measuredBatteryLossKwh(day, chargedKwh, dischargedKwh) {
...
function buildEnergeticDayTotals(day) {
    ...
    if (!autonomyConsistent)
        notesDe.push("Autarkie nicht vollständig berechenbar: Hausverbrauch und Netzbezug haben unterschiedliche Messabdeckung oder Bilanzgrenzen.");
    if (!day.complete)
        notesDe.push("Laufender Tag: Werte sind eine Zwischenbilanz.");
    if (!day.evaluable)
        notesDe.push(`Day-Telemetry noch nicht bewertbar (${round1(day.coveragePct)} % Abdeckung).`);
    if (batteryLoss.reason)
        notesDe.push(batteryLoss.reason);
    if ((immersion.energyKwh ?? 0) > 0 && immersion.pvKwh === null ||
...
function strictPvAggregate(days, energy, pv) {
...
function sumEnergeticDays(days, meta) {
    const available = days.filter((day) => !!day);
    ...
    const autonomyNonGrid = sumField(available, (day) => finite(day.autonomyConsumptionBasisKwh) && finite(day.autonomyGridImportBasisKwh)
    ...
    const fastModeIncomplete = available.some((day) => (day.evChargedKwh ?? 0) > 0 && !finite(day.evFastChargedKwh));
    ...
    if (available.length === 0)
        notesDe.push("Für diesen Zeitraum liegt noch keine Day-Telemetry vor.");
    else if (available.length < expectedDays)
        notesDe.push(`${available.length} von ${expectedDays} Tagen enthalten Energiemesswerte.`);
...
exports.sumEnergeticDays = sumEnergeticDays;
