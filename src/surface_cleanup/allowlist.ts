...
import { WALLBOX_EV_FOUNDATION_PUBLIC_STATE_IDS } from "../addons/wallbox/ev_foundation/ensure_states";
...
function isWallboxRuntimeBallast(relativeId: string): boolean {
	const prefix = "addons.wallbox.runtime.";
	if (!relativeId.startsWith(prefix)) return false;
...
const FORECAST_PLAN_BALLAST_RE =
	/^planner\.intent\.forecast_plan\.(horizon_start|horizon_end|slot_minutes|active_contributors_json|excluded_contributors_json|days_json|slots_json|contributions_json|plan_json)$/;
const DAILY_PLAN_BALLAST_RE =
	/^planner\.intent\.daily_plan\.(global_mode|slot_minutes|active_contributions_json|excluded_contributions_json|slots_json|allocations_json|totals_json|unallocated_json|policy_snapshot_json|constraint_snapshot_json)$/;
const FLEXIBLE_CONTRIB_BALLAST_RE =
	/^planner\.intent\.contributions\.(flexible\.(generated_at|contributions_json|active_json|excluded_json)|battery(\.|$)|wallbox(\.|$))/;
const LEARNING_HISTORY_JSON_RE =
	/^learning\.(thermal_boiler|thermal_runtime)\.history_json$/;
...
const WEATHER_KPI_BALLAST_RE =
	/^learning\.weather\.(cloud_bias_pct|rain_bias_mm|wind_bias_kmh|valid_fields|missing_fields|quality_level|forecast_source|actual_source|summary_yesterday|sample_days_7d)$/;
...
const THERMAL_BOILER_BALLAST_RE =
	/^learning\.thermal_boiler\.(next_run|trigger_source|history_points|cooling_asymptote_source|cooling_segments|vessel|hard_relevance|soft_relevance)$/;
const THERMAL_RUNTIME_BALLAST_RE =
	/^learning\.thermal_runtime\.(runtime_hours_median|cooling_asymptote_source|by_season_json|vessel|hard_relevance|soft_relevance)$/;
const USER_INTENT_MIRROR_BALLAST_RE =
	/^user_intent\.(wallbox|thermal|battery)\.(revision|last_changed|source_summary)$/;
const POLICY_BALLAST_RE =
	/^policy\.(system\.(schema_version|engine_version|registered_providers_json)|global\.provenance_json)$/;
const LIVE_WALLBOX_RE = /^live\.wallbox(\.|$)/;
const OPERATOR_NOTIFICATION_BALLAST_RE =
	/^operator\.notification\.(last_kind|last_dedup_key|candidates_json)$/;
const GLOBAL_MODES_BALLAST_RE = /^global_modes\.(available_json|issues_json)$/;
const IMMERSION_RUNTIME_BALLAST_RE =
	/^addons\.immersion_heater\.runtime\.(snapshot_json|planning_min_temp_c|forecast_target_temp_c|daily_plan_slot_start|daily_plan_slot_end|mandatory_allocated_power_w|flexible_allocated_power_w|minimum_runtime_remaining_sec|minimum_pause_remaining_sec|last_switch_at|daily_plan_revision|allocation_status)$/;
...
const BACKUP_BALLAST_RE =
	/^backup\.(last_kind|last_size_bytes|last_sha256|schema_version|restore\.(plan_expires_at|archive_sha256|transaction_id))$/;
const ECONOMICS_BALLAST_RE = /^economics\.(month|year)\.savings_eur$/;
const OPERATOR_STRATEGY_JSON_RE = /^operator\.plan\.strategy_json$/;
const BATTERY_LEARNING_BALLAST_RE =
	/^learning\.battery_runtime\.(avg_night_discharge_pct|avg_discharge_power_w)$/;
const PLANNER_LAST_JSON_RE = /^planner\.intent\.(last_json|last_reason_de|battery(\.|$))/;
const WALLBOX_STATUS_BALLAST_RE =
	/^addons\.wallbox\.status\.(charging_mode|charging_mode_label|vehicle_soc_pct)$/;
const WALLBOX_EVCC_BALLAST_RE =
	/^addons\.wallbox\.status\.evcc\.(snapshot_json|configured_phases|min_current_a|battery_discharge_control|connection|vehicle_range_km|vehicle_odometer_km|charge_remaining_duration_s|effective_max_current_a|effective_min_current_a|offered_current_a|charge_currents_json|charge_voltages_json|session_price|session_price_per_kwh|vehicle_detection_active|vehicle_title|smart_cost_limit|smart_cost_active)$/;
...
const BATTERY_KEEP_RE =
	/^addons\.battery\.(identity|telemetry|status\.(telemetry_ready|effective_execution_mode|state|reason|fault|lockout)|runtime\.(action|state|ownership_active|price_hold_until_iso|price_hold_heartbeat_iso|price_target_soc_pct|decision_source|reason_de|daily_plan_status|daily_plan_valid|daily_plan_revision|allocated_charge_power_w|energy_source|battery_setpoint_owner|battery_setpoint_kind|battery_setpoint_w)|diagnostics\.(fault_code|fault_reason)|grid_balance\.(enabled|active|ready|block_reason|current_price_ct_kwh|price_min_ct_kwh|price_allowed|grid_power_w|requested_power_w|requested_discharge_w|effective_power_w|hold_detected|ev_conflict|last_action|explain|live_test_armed|live_test_armed_at|live_test_result|policy_excluded_load_w|policy_excluded_reason_de)|control\.fault_reset|failsafe\.)/;
...
function isWallboxFoundationBallast(relativeId: string): boolean {
...
function isBatteryBallast(relativeId: string): boolean {
	if (!relativeId.startsWith("addons.battery.")) return false;
	if (BATTERY_KEEP_RE.test(relativeId)) return false;
...
export function isAllowlistedCleanupRelativeId(relativeId: string): boolean {
	...
	if (
		LEARNING_HISTORY_JSON_RE.test(relativeId) ||
	...
	if (
		USER_INTENT_MIRROR_BALLAST_RE.test(relativeId) ||
