...
exports.formatOperationalAssessmentDe = exports.buildOperationalAssessment = void 0;
...
function strDetail(c, key) {
    const v = contribDetail(c, key);
    return typeof v === "string" && v.trim() ? v.trim() : null;
}
function boolDetail(c, key) {
    const v = contribDetail(c, key);
...
function matchesConsumer(consumerId, allocationId) {
    if (!consumerId)
        return true;
    return allocationId === consumerId;
}
function allocActive(plan, kinds, nowMs, opts) {
    if (!plan)
        return false;
    ...
    for (const a of plan.allocations) {
        if (!kinds.includes(a.kind))
            continue;
        if (!matchesConsumer(opts.consumerId, a.consumerId))
            continue;
        if (!(a.allocatedPowerW >= ON_W || a.allocatedEnergyKwh > 0.02))
            continue;
        ...
        if (opts.current && current)
            return true;
        if (opts.today && (current || future) && dayKey(start, tz) === today)
            return true;
        if (opts.later && future && dayKey(start, tz) > today)
            return true;
    }
    return false;
}
function firstAllocWindow(plan, kinds, nowMs, timezone, consumerId) {
    if (!plan)
        return null;
    ...
    for (const a of plan.allocations) {
        if (!kinds.includes(a.kind))
            continue;
        if (!matchesConsumer(consumerId, a.consumerId))
            continue;
        if (!(a.allocatedPowerW >= ON_W || a.allocatedEnergyKwh > 0.02))
            continue;
        const start = Date.parse(a.slot.startIso);
        const end = Date.parse(a.slot.endIso);
        if (!Number.isFinite(start) || !Number.isFinite(end) || end <= nowMs)
            continue;
...
function dayPriceStats(input, day, timezone) {
    if (!input?.prices?.slots?.length)
        return null;
    let minCt = Infinity;
    let minStart = "";
    let minEnd = "";
    ...
    for (const s of input.prices.slots) {
        ...
        if (ct == null || !Number.isFinite(ct) || !Number.isFinite(start))
            continue;
        if (dayKey(start, timezone) !== day)
            continue;
        n += 1;
        if (ct < minCt) {
            minCt = ct;
    ...
    if (n < MIN_PRICED_SLOTS_FOR_DAY || !Number.isFinite(minCt))
        return null;
...
function assessEv(input) {
    ...
    if (connected === false && !hard) {
        ...
        if (goodPvTomorrow)
            next = `Morgen bei PV-Überschuss sinnvoll (${fmtKwh(pvTomorrow)} kWh erwartet), sobald das Auto da ist.`;
        else if (tomorrowPrices == null)
...
function assessImmersion(input) {
    ...
    const atTarget = live.autoTargetReached ||
...
function climateUnitsFrom(input) {
    const units = [];
    ...
    for (const [idx, c] of sorted) {
        ...
        const next = plannedToday && window
            ? preCool
...
function userGbBlock(reason, priceAllowed, minCt, nowCt) {
    const r = (reason ?? "").toLowerCase();
    if (priceAllowed === false || r.includes("price") || r.includes("preis")) {
        const bit = nowCt != null && minCt != null
            ? ` (${fmtCt(nowCt)} ct, Freigabe ab ${fmtCt(minCt)} ct)`
            : "";
        return `Aktuell gesperrt – Strompreis unter Freigabegrenze${bit}.`;
    }
    if (r.includes("soc") || r.includes("reserve"))
        return "Aktuell gesperrt – Reserve/SOC-Grenze.";
    if (r.includes("ev") || r.includes("wallbox") || r.includes("hold"))
        return "Aktuell gesperrt – Fahrzeugladung hat Vorrang.";
    if (r.includes("ready") || r.includes("mapping") || r.includes("stale"))
        return "Aktuell gesperrt – technische Freigabe fehlt.";
    if (reason && reason.trim() && !/[._]/.test(reason) && reason.length < 80)
        return `Aktuell gesperrt – ${reason.trim()}`;
    return "Netzausgleich aktuell nicht freigegeben.";
}
function assessGridBalance(input) {
    const gb = input.gb;
...
function assessForecast(input) {
    const bits = [];
    if (input.pvTodayKwh != null)
        bits.push(`Heute ${fmtKwh(input.pvTodayKwh)} kWh PV erwartet`);
    if (input.pvTomorrowKwh != null)
        bits.push(`morgen ${fmtKwh(input.pvTomorrowKwh)} kWh`);
    ...
    if (bits.length === 0)
        return "Forecast noch nicht vollständig.";
    return `${bits.join(", ")}.`;
}
 ...
 */
function buildOperationalAssessment(input) {
...
exports.buildOperationalAssessment = buildOperationalAssessment;
function formatOperationalAssessmentDe(a) {
    const lines = ["EMS-Einschätzung"];
...
exports.formatOperationalAssessmentDe = formatOperationalAssessmentDe;
