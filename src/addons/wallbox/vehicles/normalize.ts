import type { VehicleProfileSource, WallboxVehicleProfile } from "./types";
import type { WallboxVehicleProfileInput } from "./config";
import { normalizeEvccMatchToken, sanitizeVehicleId } from "./vehicle_id";

export interface NormalizeWallboxVehicleProfileResult {
...
function parseSource(raw: unknown): VehicleProfileSource {
	const s = String(raw ?? "").trim().toLowerCase();
	if (s === "evcc" || s === "hybrid" || s === "manual") return s;
	return "manual";
}

function parseOptionalPositiveNumber(raw: unknown, field: string, invalid: string[]): number | null {
...
function parseOptionalNonNegativeNumber(raw: unknown, field: string, invalid: string[]): number | null {
	if (raw === null || raw === undefined || raw === "") return null;
	const n = typeof raw === "number" ? raw : parseFloat(String(raw).replace(",", "."));
	if (!Number.isFinite(n)) {
		invalid.push(field);
...
function parseOptionalSoc(raw: unknown, field: string, invalid: string[]): number | null {
	if (raw === null || raw === undefined || raw === "") return null;
	const n = typeof raw === "number" ? raw : parseFloat(String(raw).replace(",", "."));
	if (!Number.isFinite(n)) {
		invalid.push(field);
...
function parseOptionalEfficiency(raw: unknown, field: string, invalid: string[]): number | null {
	if (raw === null || raw === undefined || raw === "") return null;
	const n = typeof raw === "number" ? raw : parseFloat(String(raw).replace(",", "."));
	if (!Number.isFinite(n)) {
		invalid.push(field);
...
function parsePhases(raw: unknown, field: string, invalid: string[]): number[] {
...
function parsePreferredPhases(raw: unknown, field: string, invalid: string[]): number | null {
	if (raw === null || raw === undefined || raw === "") return null;
...
function validateSocOrdering(profile: WallboxVehicleProfile, invalid: string[]): void {
...
function validateCurrentOrdering(profile: WallboxVehicleProfile, invalid: string[]): void {
...
export function normalizeWallboxVehicleProfile(
	input: WallboxVehicleProfileInput,
	nowIso: string,
): NormalizeWallboxVehicleProfileResult {
	const invalidFields: string[] = [];
	const reasons: string[] = [];

	const idResult = sanitizeVehicleId(input.vehicleId);
	...
	const profile: WallboxVehicleProfile = {
		vehicleId: idResult.id,
		displayName,
		enabled,
		isGuest,
		source,
		evccVehicleId: normalizeEvccMatchToken(input.evccVehicleId),
		evccVehicleName: normalizeEvccMatchToken(input.evccVehicleName),
		batteryCapacityNetKwh: parseOptionalPositiveNumber(
			input.batteryCapacityNetKwh,
			"batteryCapacityNetKwh",
			invalidFields,
		),
		...
		referenceRangeAt100PctKm: parseOptionalPositiveNumber(
			input.referenceRangeAt100PctKm,
			"referenceRangeAt100PctKm",
			invalidFields,
		),
		socFallbackMaxAgeMin: parseOptionalNonNegativeNumber(
			input.socFallbackMaxAgeMin,
			"socFallbackMaxAgeMin",
			invalidFields,
		),
...
export function normalizeWallboxVehicleProfiles(
	inputs: WallboxVehicleProfileInput[],
	nowIso: string,
): { profiles: WallboxVehicleProfile[]; errors: Array<{ slotIndex: number; reasons: string[] }> } {
	const profiles: WallboxVehicleProfile[] = [];
	const errors: Array<{ slotIndex: number; reasons: string[] }> = [];
