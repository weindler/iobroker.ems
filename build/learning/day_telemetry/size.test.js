...
function buildRealisticDay(dateKey, tz) {
    const layout = (0, slots_js_1.buildDaySlotLayout)(dateKey, tz);
    const day = (0, types_js_1.emptyDayRecord)(dateKey, tz, layout.startMs, layout.endMs, layout.slotCount);
    ...
    for (let r = 0; r < REPLANS_PER_DAY; r++) {
        ...
        if (slotIdx >= 0)
            day.buckets.snapshotIdRef[slotIdx] = snap.id;
    }
    ...
    for (let i = 0; i < layout.slotCount; i++) {
        ...
        let mask = (0, quality_mask_js_1.encodeDomainQuality)(0, quality_mask_js_1.TELEMETRY_DOMAIN.PV, quality_mask_js_1.DOMAIN_QUALITY.ok);
...
(0, node_test_1.describe)("day_telemetry realistischer Größentest (Speicher-Kompaktierung)", () => {
    (0, node_test_1.it)("17) realistischer Einzeltag — 192-Slot-Horizont × 180 Replans bleibt weit unter 1 MiB", async () => {
        const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daytel-realday-"));
        try {
            const { day } = buildRealisticDay("2026-08-30", "Europe/Berlin");
            strict_1.default.equal(day.forecastSnapshots.length, REPLANS_PER_DAY);
            await (0, persist_js_1.writeDayTelemetryDay)(dir, day);
            const st = await fs.stat(path.join(dir, "2026-08-30.json"));
            console.log(`day_telemetry realistischer Tag (${REPLANS_PER_DAY} Replans, ${HORIZON_SLOTS}-Slot-Horizont): ${st.size} bytes (${(st.size / 1024).toFixed(1)} KiB)`);
            strict_1.default.ok(st.size < 512 * 1024, `Realistischer Tag zu groß: ${st.size} bytes — Kompaktierung wirkt nicht wie erwartet`);
            /* Punkt-in-Zeit-Rekonstruktion: Roundtrip muss verlustfrei sein (keine Runtime-Rekonstruktion). */
            const reloaded = await (0, persist_js_1.readDayTelemetryDay)(dir, "2026-08-30");
            strict_1.default.ok(reloaded);
            strict_1.default.equal(reloaded.forecastSnapshots.length, REPLANS_PER_DAY);
            for (const idx of [0, 1, 39, 40, 41, 100, REPLANS_PER_DAY - 1]) {
                const before = day.forecastSnapshots[idx];
                const after = reloaded.forecastSnapshots.find((s) => s.id === before.id);
                strict_1.default.ok(after, `Snapshot ${before.id} nach Reload nicht auffindbar`);
                strict_1.default.equal(after.priceSlots.length, HORIZON_SLOTS);
                strict_1.default.equal(after.pvSlotKwh.length, HORIZON_SLOTS);
                strict_1.default.deepEqual(after.priceSlots, before.priceSlots, `priceSlots bei Snapshot ${idx} nicht identisch rekonstruiert`);
                strict_1.default.deepEqual(after.pvSlotKwh, before.pvSlotKwh, `pvSlotKwh bei Snapshot ${idx} nicht identisch rekonstruiert`);
            }
            /* replanEvents bleiben eindeutig einem damals gültigen Snapshot zuordenbar. */
            for (const ev of reloaded.replanEvents) {
                strict_1.default.ok(reloaded.forecastSnapshots.some((s) => s.id === ev.snapshotId), `replanEvent ${ev.planId} referenziert unbekannten Snapshot ${ev.snapshotId}`);
            }
            /* Basisrevisionen bleiben klein — Größenordnung der echten Revisionen, keine 180 Kopien. */
            strict_1.default.ok((reloaded.forecastRevisions?.length ?? 0) <= Math.ceil(REPLANS_PER_DAY / REVISION_EVERY_N_REPLANS) + 2);
            strict_1.default.ok((reloaded.forecastRevisions?.length ?? 0) >= 2, "echte Preisrevisionen müssen sichtbar bleiben");
        }
        finally {
            await fs.rm(dir, { recursive: true, force: true });
        }
    });
    (0, node_test_1.it)("16) 90 Tage realistische Replan-Last — Gesamtgröße skaliert sinnvoll für Retention", async () => {
        ...
        try {
            ...
            console.log(`day_telemetry 90 realistische Tage (${REPLANS_PER_DAY} Replans/Tag, ${HORIZON_SLOTS}-Slot-Horizont): ` +
             ...
             */
            strict_1.default.ok(mib < 30, `90-Tage-Retention mit realistischer Replan-Last zu groß: ${mib.toFixed(3)} MiB`);
        }
        finally {
            await fs.rm(dir, { recursive: true, force: true });
        }
