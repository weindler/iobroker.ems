...
exports.planBatteryPriceBridge = void 0;
function unavailable(reasonDe) {
...
function planBatteryPriceBridge(input) {
    if (input.mode === "off")
        return unavailable("Batterieoptimierung ausgeschaltet.");
    if (input.priceSource !== "dynamic_tariff")
        return unavailable("Keine veröffentlichten Tibber-Preise für eine verbindliche Netzladung.");
    ...
    if (cap == null || cap <= 0 || input.socPct == null || input.minSocPct == null || input.maxSocPct == null ||
        ...
        input.pvConfidencePct == null)
        return unavailable("SOC, Nachtverbrauch, Wirkungsgrad oder Batteriekosten fehlen.");
    ...
    if (publishedEnd < 4)
        return unavailable("Veröffentlichte Preise reichen nicht für eine Netzladeentscheidung.");
    const confidence = Math.max(0.2, Math.min(1, input.pvConfidencePct / 100));
    const surplus = (s) => Math.max(0, s.pvKwh * confidence - s.houseKwh - Math.max(0, s.committedKwh));
    ...
    for (let i = 1; i + 1 < slots.length; i++) {
        if (surplus(slots[i]) <= 0 || surplus(slots[i + 1]) <= 0)
            continue;
        const following = slots.slice(i, Math.min(slots.length, i + 48)).reduce((n, s) => n + surplus(s), 0);
        if (following >= Math.max(1, input.nightKwh * 0.5)) {
            recovery = i;
            break;
        }
    }
    ...
    if (!withDemand.length)
        return unavailable("Kein Netzbedarf vor der nächsten verlässlichen PV-Erholung.");
    ...
    const cheap = slots.slice(0, peakIndex).filter(s => s.gridAllowed && s.importCt != null)
        .reduce((a, b) => !a || b.importCt < a.importCt ? b : a, null);
    if (!cheap)
        return unavailable("Kein erlaubtes Ladefenster vor dem teuren Netzbedarf.");
    const cost = cheap.importCt / (etaC * etaD) + input.wearCtPerKwh;
    const safetyMargin = Math.max(2, cost * 0.05);
    if (peak.importCt <= cost + safetyMargin)
        return unavailable("Preisvorteil nach Verlusten und Verschleiß nicht ausreichend.");
    ...
    const targetStored = Math.min(cap * input.maxSocPct / 100, cap * input.minSocPct / 100 + Math.max(input.nightKwh, totalDemand / etaD, highDemand / etaD) * uncertainty);
...
exports.planBatteryPriceBridge = planBatteryPriceBridge;
