...
exports.countWallboxFeedbackExpectations = exports.evaluateWallboxFeedback = exports.normalizeWallboxFeedbackValue = exports.buildWallboxFeedbackContract = void 0;
const control_object_meta_1 = require("./control_object_meta");
const feedback_config_1 = require("./feedback_config");
function issueKindForReason(reason) {
    if (!reason)
        return "none";
    if (reason.includes("cross_controller"))
        return "cross_controller";
    ...
    if (reason.includes("unsupported") || reason.includes("hold_feedback"))
        return "unsupported";
    if (reason.includes("mapping") || reason.includes("timing"))
        return "mapping";
    return "unsupported";
}
function isCrossControllerFeedback(writeTargetStateId, readbackStateId) {
    const writeKind = (0, control_object_meta_1.classifyWallboxControlTargetKind)(writeTargetStateId);
    const readKind = (0, control_object_meta_1.classifyWallboxControlTargetKind)(readbackStateId);
    if (writeKind === readKind)
        return false;
    if (writeKind === "user_configured" || readKind === "user_configured")
        return false;
    return true;
}
function toleranceForRole(role, config) {
    if (role === "set_max_current_a" || role === "set_current_a") {
        return config.maxCurrentToleranceA;
    }
    return null;
}
function emptyContract(status, issueKind, blockReason, createdAt, controlModel = "none") {
    return {
        required: false,
        ready: false,
...
function expectationFromOperation(op, config) {
 ...
 */
function buildWallboxFeedbackContract(input) {
    const { writePlan, feedbackConfig, now } = input;
    const createdAt = now.toISOString();
    if (!writePlan) {
        return emptyContract("unavailable", "unavailable", "write_plan_missing", createdAt);
    }
    const timing = (0, feedback_config_1.validateWallboxFeedbackTiming)(feedbackConfig);
    if (!timing.valid) {
        return {
...
exports.buildWallboxFeedbackContract = buildWallboxFeedbackContract;
function normalizeWallboxFeedbackValue(input) {
...
exports.normalizeWallboxFeedbackValue = normalizeWallboxFeedbackValue;
function valuesMatch(expected, actual, expectedType, tolerance) {
...
function evaluateExpectation(exp, rawActual, elapsedMs, settleTimeMs, timeoutMs, objectMeta) {
    ...
    const matched = valuesMatch(exp.expectedValue, normalized.value, exp.expectedValueType, exp.tolerance);
    if (matched) {
        return {
            ...exp,
...
function aggregateStatus(expectations, required) {
 ...
 */
function evaluateWallboxFeedback(input) {
    ...
    const expectations = contract.expectations.map((exp) => evaluateExpectation(exp, actualValues[exp.readbackStateId], elapsedMs, settleTimeMs, timeoutMs));
...
exports.evaluateWallboxFeedback = evaluateWallboxFeedback;
function countWallboxFeedbackExpectations(expectations) {
    const counts = {
        matched: 0,
        mismatch: 0,
        unavailable: 0,
...
exports.countWallboxFeedbackExpectations = countWallboxFeedbackExpectations;
