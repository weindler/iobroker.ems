...
export const STATE_SURFACE_FAMILIES: StateSurfaceFamily[] = [
	...
	},
	{
		...
		idPattern: "system.* | live.* | execution.safety.* | economics.* | profiles.catalog_json | app.* | operator.briefing_de | operator.assessment.* | operator.outlook_72h* | operator.diagnostics.*",
		estimatedStaticCount: 46,
		dataTypes: ["string", "number", "boolean"],
		readWrite: "mostly_read",
		producer: "ems_light/ensure_states, tick",
		consumers: ["VIS (partial)", "operator UI", "local app/API clients"],
