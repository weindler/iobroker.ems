...
function minSocInWindow(
	day: DayTelemetryDayRecord,
	fromMs: number,
	toMs: number,
): { min: number | null; coveredSlots: number; totalSlots: number } {
	const layout = buildDaySlotLayout(day.dateKey, day.timezone);
	let min: number | null = null;
	let covered = 0;
	let total = 0;
	for (const slot of layout.slots) {
		if (slot.endMs <= fromMs || slot.startMs >= toMs) continue;
		total++;
 ...
 */
export function evaluateBatteryFindings(
	day: DayTelemetryDayRecord,
	nextDay: DayTelemetryDayRecord | null,
): EvaluatorFinding[] {
	const findings: EvaluatorFinding[] = [];

	let bestSnap: (typeof day.forecastSnapshots)[number] | null = null;
	let bestMs = -Infinity;
	for (const s of day.forecastSnapshots) {
		if (s.batteryDecision?.requiredSocAtPvEndPct == null) continue;
	...
	if (nextDay && toMs > day.endMs) {
		...
		if (nextWindow.min != null && (min == null || nextWindow.min < min)) min = nextWindow.min;
