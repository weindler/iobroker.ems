...
export type PlannerCandidateComparisonStatus =
	| "matched"
	| "mismatch"
	| "not_comparable"
	| "validation_failed"
...
export interface PlannerCandidateComparisonResult {
	status: PlannerCandidateComparisonStatus;
...
export function comparePlanCandidates(
	reference: PlannerPlanCandidate,
	worker: PlannerPlanCandidate,
): PlannerCandidateComparisonResult {
	...
	if (
		reference.horizonStart !== worker.horizonStart ||
	...
	for (let i = 0; i < slotCount; i++) {
		...
		if (!a || !b) {
			...
			compareField(
				`forecastSlots[${i}].houseLoadPowerW`,
				"forecast",
				a.houseLoadPowerW,
				b.houseLoadPowerW,
				mismatches,
			...
			compareField(
				`forecastSlots[${i}].gridPriceCtPerKwh`,
				"price",
				a.gridPriceCtPerKwh,
				b.gridPriceCtPerKwh,
				mismatches,
			...
			compareField(
				`forecastSlots[${i}].gridImportAllowed`,
				"constraint",
				a.gridImportAllowed,
				b.gridImportAllowed,
				mismatches,
			...
			compareField(
				`forecastSlots[${i}].gridMaxImportPowerW`,
				"constraint",
				a.gridMaxImportPowerW,
				b.gridMaxImportPowerW,
				mismatches,
			);
		}
	...
	compareField(
		"totals.flexibleAllocatedEnergyKwh",
		"totals",
		reference.totals.flexibleAllocatedEnergyKwh,
		worker.totals.flexibleAllocatedEnergyKwh,
		mismatches,
	);
