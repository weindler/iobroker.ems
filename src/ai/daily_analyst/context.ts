...
import type { DomainScore, EvaluationRecord, EvaluatorFinding } from "../../learning/daily_evaluator/types";
import type { EconomicsDayRecord } from "../../economics/types";
import type { ShadowDayRecord } from "../../learning/shadow_engine/types";

export type AiAnalystContext = {
	schemaVersion: 1;
	purpose: "daily_analyst_findings";
	dateKey: string;
	globalScore: number | null;
	scores: Array<{ topic: string; value: number | null; sampleCount: number }>;
	eligibility: Array<{ domain: string; status: string; coveragePct: number }>;
	findings: Array<{
		domain: string;
		eventType: string;
...
export function buildAiAnalystContext(input: {
	dateKey: string;
	record: EvaluationRecord;
	findings: EvaluatorFinding[];
	economics: EconomicsDayRecord | null;
	shadow: ShadowDayRecord | null;
}): AiAnalystContext {
	const scores: Array<{ topic: string; value: number | null; sampleCount: number }> = input.record.scores.map(
		(s: DomainScore) => ({ topic: s.topic, value: s.value, sampleCount: s.sampleCount }),
	);
