...
import type { DeviceClass } from "../profiles/types";
import { isEvActuallyCharging } from "../addons/wallbox/charge_hold";

export const APP_CORE_CONTRACT_STATE = "app.core_contract_json";
export const APP_CORE_SNAPSHOT_STATE = "app.core_snapshot_json";

export const APP_CORE_CONTRACT = {
	schemaVersion: 1,
	snapshotStateId: APP_CORE_SNAPSHOT_STATE,
...
type AppDeviceClass = DeviceClass | "grid_balance";

export interface AppDeviceSnapshot {
	id: string;
...
export interface AppCoreSnapshot {
	...
	planner: {
		...
		reasonDe: string | null;
		outlook72h: JsonObject | null;
	};
	profiles: {
		catalogStateId: "profiles.catalog_json";
		catalog: JsonObject | null;
	};
	devices: AppDeviceSnapshot[];
	learning: {
		batteryNight: JsonObject;
		thermal: JsonObject;
		gridBalanceEconomics: JsonObject;
	};
	statistics: {
		periodId: string | null;
		energyToday: JsonObject | null;
		energyPeriod: JsonObject | null;
		economics: JsonObject;
	};
	safety: {
		batteryGridChargeActive: boolean | null;
		evccNowActive: boolean | null;
		externalFastChargeActive: boolean | null;
		gridBalanceActive: boolean | null;
		gridBalanceMustBeOff: boolean | null;
		gridBalanceInvariantSatisfied: boolean | null;
	};
	dataQuality: {
		missingStateIds: string[];
		invalidJsonStateIds: string[];
		tickHints: string[];
	};
}

export interface AppCoreSnapshotHost {
	config?: unknown;
...
export function appCoreSnapshotStateIds(): string[] {
	const ids = [...BASE_STATE_IDS] as string[];
	for (let unit = 1; unit <= 5; unit++) {
		for (const field of CLIMATE_FIELDS) ids.push(`addons.air_conditioning.units.unit_${unit}.${field}`);
		for (const field of CLIMATE_LEARNING_FIELDS) ids.push(`learning.climate_thermal.unit_${unit}.${field}`);
	}
	return ids;
}
...
function objectValue(value: unknown, id: string, invalidJsonStateIds: string[]): JsonObject | null {
	if (value === null || value === undefined || value === "") return null;
	try {
...
function decisionFor(outlook: JsonObject | null, kind: string, consumerId?: string): JsonObject | null {
	...
	for (const raw of decisions) {
		...
		if (consumerId && decision.consumerId !== consumerId) continue;
		return decision;
	}
	return null;
}

function configText(config: unknown, key: string): string | null {
	if (!config || typeof config !== "object") return null;
	return textValue((config as Record<string, unknown>)[key]);
}

function climateDevices(
	values: Readonly<Record<string, unknown>>,
	config: unknown,
	outlook: JsonObject | null,
): AppDeviceSnapshot[] {
	const devices: AppDeviceSnapshot[] = [];
	for (let unit = 1; unit <= 5; unit++) {
		...
		devices.push({
			...
			labelDe: name,
			templateId:
				configText(config, `ac_u${unit}_profile`) ?? configText(config, "ac_default_profile"),
			current: {
			...
			reasonDe:
				textValue(values[`${base}.allocation_reason_de`]) ?? textValue(values[`${base}.reason_de`]),
		});
	}
	return devices;
}

export function buildAppCoreSnapshot(args: {
	generatedAtIso: string;
	health: string | null;
	executionMode: string | null;
	tickHints?: string[];
	...
	const { values } = args;
	const invalidJsonStateIds: string[] = [];
	...
	const evActuallyCharging = (() => {
		if (isEvActuallyCharging({ charging: evCharging, chargePowerW: evChargePowerW })) return true;
		if (evCharging === false || evChargePowerW !== null) return false;
		return null;
	})();
	const evccNowActive = (() => {
		if (evConnected === false) return false;
		if (evccMode !== null && evccMode.toLowerCase() !== "now") return false;
		if (evConnected === null || evccMode === null || evActuallyCharging === null) return null;
	...
	const externalFastChargeActive = (() => {
		...
		if (batteryHold !== null) return batteryHold;
		if (evConnected === false) return false;
		...
		if (external === null && rewards === null && evccNowActive === null) return null;
	...
	const gridBalanceMustBeOff =
		batteryGridChargeActive === null && evccNowActive === null && externalFastChargeActive === null
			? null
			: batteryGridChargeActive === true || evccNowActive === true || externalFastChargeActive === true;
	const gridBalanceInvariantSatisfied =
		gridBalanceMustBeOff !== true
			? null
	...
	const devices: AppDeviceSnapshot[] = [
		...
		},
		{
			...
			reasonDe:
				textValue(values["addons.immersion_heater.runtime.reason_de"]) ??
		...
		},
		{
			...
			reasonDe:
				textValue(values["addons.battery.grid_balance.explain"]) ??
...
export async function collectAppCoreSnapshot(
	host: AppCoreSnapshotHost,
	args: { generatedAtIso: string; health: string | null; executionMode: string | null; tickHints?: string[] },
): Promise<AppCoreSnapshot> {
	const values: Record<string, unknown> = {};
	const missingStateIds: string[] = [];
	await Promise.all(
		appCoreSnapshotStateIds().map(async (id) => {
			try {
				const state = await host.getStateAsync(id);
				if (!state) missingStateIds.push(id);
...
export async function publishAppCoreSnapshot(
	host: AppCoreSnapshotHost,
	args: { generatedAtIso: string; health: string | null; executionMode: string | null; tickHints?: string[] },
): Promise<AppCoreSnapshot> {
	const snapshot = await collectAppCoreSnapshot(host, args);
	await host.setStateAsync(APP_CORE_SNAPSHOT_STATE, {
