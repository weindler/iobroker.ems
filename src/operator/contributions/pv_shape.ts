...
export interface PvShapeSlotInput {
	startIso: string;
	endIso: string;
}

export interface PvShapeHourPoint {
	/** Stundenbeginn (UTC-ISO), z. B. BrightSky `hourly.NN.timestamp`. */
	hourStartIso: string;
	cloudPct: number | null;
	solarEstimateKwh: number | null;
}

export interface PvShapeSlotResult {
...
export function clearSkyWeight(elevationDeg: number): number {
	if (elevationDeg <= 0) return 0;
	return Math.sin(degToRad(elevationDeg));
}

function hourBucketMs(iso: string): number {
 ...
 */
export function buildPvShapeForDay(
	slots: PvShapeSlotInput[],
	...
	if (dailyKwh === null || !Number.isFinite(dailyKwh) || dailyKwh <= 0) return [];
	if (latDeg === null || lonDeg === null || !Number.isFinite(latDeg) || !Number.isFinite(lonDeg)) return [];

	const hourlyByBucket = new Map<number, PvShapeHourPoint>();
	for (const p of hourlyPoints) {
		const ms = Date.parse(p.hourStartIso);
		if (Number.isFinite(ms)) hourlyByBucket.set(hourBucketMs(p.hourStartIso), p);
	}
	...
	const hourWeightSums = new Map<number, number>();
	slots.forEach((s, i) => {
	...
	const dampedWeights = slots.map((s, i) => {
		...
		if (point.solarEstimateKwh !== null && point.solarEstimateKwh >= 0) {
			...
			if (hourSum > 0) return raw * (point.solarEstimateKwh / hourSum);
		}
	...
	const slotHours = slots.map(
		(s) => Math.max(0, Date.parse(s.endIso) - Date.parse(s.startIso)) / 3_600_000,
	);
