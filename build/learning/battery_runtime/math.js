...
function median(values) {
    ...
    return round3(sorted.length % 2 === 0
...
function computeNightDischarges(params) {
    ...
    function scoreWindows(windows) {
        ...
        for (const w of windows) {
            ...
            const minPoint = (0, night_bridge_1.findMinSocPointInRange)(params.socPoints, obs.startTs, obs.endTs);
            const socEnd = minPoint?.socPct ??
        ...
        return {
            avgPct: (0, night_bridge_1.weightedAverage)(trimmed.map((n) => n.pct), trimmed.map((n) => n.weight)),
            avgKwh: params.capacityKwh !== null && kwhRecords.length === trimmed.length
                ? (0, night_bridge_1.weightedAverage)(kwhRecords.map((n) => n.kwh), kwhRecords.map((n) => n.weight))
                : null,
            medianKwh: params.capacityKwh !== null && kwhRecords.length === trimmed.length
                ? median(kwhRecords.map((n) => n.kwh))
                : null,
            validNights: trimmed.length,
            avgBridgeHours: average(trimmed.map((n) => n.bridgeHours)),
            gridBalanceAttributedNights: trimmed.filter((n) => (n.diagnostic.gridBalanceKwh ?? 0) > 0.01).length,
            gridBalanceExcludedNights,
            nightSamples,
        };
    }
...
function trimNightOutliers(nights) {
    if (nights.length < 4)
        return nights;
 ...
 */
function computeNightHouseLoadDiagnostic(params) {
    ...
    for (const w of params.windows) {
        ...
        const houseKwh = (0, night_bridge_1.integratePowerKwh)(params.housePowerPoints, obs.startTs, obs.endTs);
    ...
    return { avgKwh: (0, night_bridge_1.weightedAverage)(values, weights), validNights: values.length };
}
exports.computeNightHouseLoadDiagnostic = computeNightHouseLoadDiagnostic;
...
function computeBatteryRuntimeLearning(params) {
     ...
     */
    const predictedNightGridImportKwh = houseLoad.avgKwh !== null && night.avgKwh !== null
