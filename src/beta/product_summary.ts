...
export function buildUnifiedDayAgendaDe(
	...
	for (const w of wb) {
		...
		const gridOptimal = cells.some(
			(a) => a.energySource === "grid" && a.reasonCodes.includes("grid_import_cost_optimal"),
		);
...
export function buildProductSummaryDe(
	...
	if (bridge) {
		if (!bridge.usable) parts.push(`Batterieplanung eingeschränkt: ${bridge.reasonDe}`);
		else {
			...
			if (bridge.holdEndIso) parts.push(`Batterieenergie bis ${fmtDayClock(bridge.holdEndIso, x.timezone) ?? "zur Hochpreisphase"} schonen; Entladung wird aktiv gesperrt.`);
		}
	}
