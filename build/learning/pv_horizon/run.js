...
async function resolveRawKwhByDay(host, rawStateIds, skipTodayTomorrow) {
    ...
    for (let i = 0; i < constants_1.PV_HORIZON_DAY_COUNT; i++) {
        ...
        if (raw === null && i >= 2) {
            raw = await (0, history_1.readStateNum)(host, `learning.price_outlook.local_pv_day${i + 1}_raw_kwh`);
        }
        values.push(raw);
    }
    return values;
...
async function clearHorizonDay(host, dayIndex) {
    ...
    for (const suffix of ["raw_kwh", "applied_bias_pct", "corrected_kwh", "confidence_pct"]) {
