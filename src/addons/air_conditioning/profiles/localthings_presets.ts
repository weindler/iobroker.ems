...
/** Bekannte Installations-Presets (nur Prefill bei Profilwahl LocalThings, nie globale Defaults). */
export type LocalthingsUnitPreset = {
	unitIndex: number;
	/** Optionaler Name-Hinweis zur Zuordnung. */
	nameHint?: string;
...
function climateMaps(entity: string): Partial<Record<AcMappingRole, string>> {
...
function switchMaps(entityBase: string, suffix: string): {
	status: string;
	on: string;
	off: string;
} {
...
function extrasFor(entityBase: string, opts?: { firmware?: boolean; muteOnce?: boolean }): Partial<Record<AcMappingRole, string>> {
	...
	const out: Partial<Record<AcMappingRole, string>> = {
...
export const LOCALTHINGS_SITE_PRESETS: LocalthingsUnitPreset[] = [
	{
		unitIndex: 1,
		nameHint: "Wohnzimmer",
		climateEntity: WZ,
 ...
 */
export function deriveLocalthingsMappingsFromClimateBase(
	climateBaseOrStateId: string,
): Partial<Record<AcMappingRole, string>> {
	...
	base = base.replace(
		/\.(state|state_boolean|temperature|current_temperature|turn_on|turn_off|set_hvac_mode|set_temperature|fan_mode|set_fan_mode|preset_mode|set_preset_mode|swing_mode|set_swing_mode|hvac_modes|fan_modes|preset_modes|min_temp|max_temp|target_temp_step)$/,
		"",
	);
...
export function isHassLocalthingsTarget(stateId: string): boolean {
	return stateId.trim().startsWith("hass.") && stateId.includes(".entities.");
}

export function isSmartThingsTarget(stateId: string): boolean {
