...
exports.normalizePresenceWindow = normalizePresenceWindow;
function inWindow(iso, startIso, endIso) {
    const t = Date.parse(iso);
    return t >= Date.parse(startIso) && t < Date.parse(endIso);
}
function findExplicit(explicit, slotStartIso) {
    if (!explicit?.length)
        return null;
...
function mergeWindows(windows) {
    ...
    for (const next of sorted.slice(1)) {
        const same = normalizePresenceWindow(cur).status === normalizePresenceWindow(next).status &&
 ...
 */
function buildVehicleAvailabilityWindows(args) {
    ...
    for (const slot of args.slots) {
        ...
        const pred = (0, vehicle_presence_1.predictAt)(args.learningStore, startMs, args.timezone, args.learningVehicleKey ?? null);
...
exports.buildVehicleAvailabilityWindows = buildVehicleAvailabilityWindows;
function presenceDigest(windows) {
...
exports.presenceDigest = presenceDigest;
 ...
 */
function evaluateVehicleGoalFeasibility(input, opts) {
    ...
    const reasonCodes = [];
    if (wb.connectedNow)
        reasonCodes.push(reason_codes_1.REASON.VEHICLE_AVAILABLE_NOW);
    else
        reasonCodes.push(reason_codes_1.REASON.VEHICLE_UNAVAILABLE_NOW);
    if (needKwh === null || needKwh <= EPS) {
        return {
            ...
            reasonCodes: [...reasonCodes, reason_codes_1.REASON.VEHICLE_GOAL_REACHABLE],
        };
    }
    if (maxAll + EPS < needKwh) {
        reasonCodes.push(reason_codes_1.REASON.VEHICLE_GOAL_UNREACHABLE);
        return {
            needKwh,
            maxFeasibleEnergyKwh: round3(maxAll),
            maxFeasibleHardEnergyKwh: round3(maxHard),
    ...
    if (maxHard + EPS >= needKwh) {
        reasonCodes.push(reason_codes_1.REASON.VEHICLE_GOAL_REACHABLE);
        return {
            needKwh,
            maxFeasibleEnergyKwh: round3(maxAll),
            maxFeasibleHardEnergyKwh: round3(maxHard),
    ...
    if (unknownHoursBeforeDeadline > EPS && maxHard + maxPred + EPS < needKwh) {
        reasonCodes.push(reason_codes_1.REASON.VEHICLE_GOAL_AT_RISK_DUE_TO_UNKNOWN_AVAILABILITY);
    ...
    if (maxHard + maxPred + EPS >= needKwh && maxHard + EPS < needKwh) {
        reasonCodes.push(reason_codes_1.REASON.VEHICLE_GOAL_AT_RISK);
        reasonCodes.push(reason_codes_1.REASON.VEHICLE_PRESENCE_PREDICTED_AVAILABLE);
        return {
            needKwh,
            maxFeasibleEnergyKwh: round3(maxAll),
            maxFeasibleHardEnergyKwh: round3(maxHard),
...
exports.evaluateVehicleGoalFeasibility = evaluateVehicleGoalFeasibility;
function round3(n) {
    return Math.round(n * 1000) / 1000;
}
/** Slot darf Fahrzeug-Allocation erhalten (nur status=available). */
function vehicleSlotAllocatable(wb, slotStartIso) {
...
exports.vehicleSlotAllocatable = vehicleSlotAllocatable;
function collectPresenceReasonCodes(windows) {
    const codes = new Set();
    for (const w of windows) {
        const n = normalizePresenceWindow(w);
        if (n.source === "live_connected")
            codes.add(reason_codes_1.REASON.VEHICLE_AVAILABLE_NOW);
        if (n.source === "live_disconnected")
            codes.add(reason_codes_1.REASON.VEHICLE_UNAVAILABLE_NOW);
        if (n.source === "explicit")
            codes.add(reason_codes_1.REASON.VEHICLE_PRESENCE_EXPLICIT);
        if (n.source === "predicted" && n.status === "available") {
            codes.add(reason_codes_1.REASON.VEHICLE_PRESENCE_PREDICTED_AVAILABLE);
        }
        if (n.source === "predicted" && n.status === "unavailable") {
            codes.add(reason_codes_1.REASON.VEHICLE_PRESENCE_PREDICTED_UNAVAILABLE);
        }
        if (n.status === "unknown")
            codes.add(reason_codes_1.REASON.VEHICLE_PRESENCE_UNKNOWN);
    }
    return [...codes];
}
exports.collectPresenceReasonCodes = collectPresenceReasonCodes;
