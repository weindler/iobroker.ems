...
**Stand:** v0.1.217 (Wetter-Horizon Tag 1–7 + Bias, analog PV)
...
| `weather_forecast` | Add-on | `learning.weather.*`, `learning.weather.horizon.day1–7.*` (Admin-Mapping + Bias) |
...
- Wetter-Kontextfelder (`weatherMinTempC`/`weatherMaxTempC`) für jeden Tag aus
  `learning.weather.horizon.day{1–7}.*` (Tag1=heute … Tag7); Bias aus Tag-1-Freeze vs.
  Live-Ist (Fallback: `learning.weather.temp_bias_c`); ohne Mapping `null` (nie Fake-0)
