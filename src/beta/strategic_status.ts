...
} from "../operator/daily_plan/unified/types";

const ON_W = 50;
const FULL_SOC_PCT = 99;

export type BatteryStrategicStatus =
	| "charge"
	| "hold"
	| "reserve_protected"
...
export type WallboxStrategicStatus =
	| "waiting_for_vehicle"
	| "waiting_for_goal"
	| "scheduled"
	| "charging"
...
export type BatteryStrategicSnapshot = {
	status: BatteryStrategicStatus;
	labelDe: string;
	reasonDe: string;
	/** Kurze Agenda-/Kartenzeile. */
	summaryDe: string;
	hasChargeAllocation: boolean;
	socPct: number | null;
	nightReserveKwh: number | null;
};

export type WallboxStrategicSnapshot = {
...
export type AddonStrategicPlanSnapshot = {
	schemaVersion: 1;
	generatedAtIso: string;
	battery: BatteryStrategicSnapshot;
	wallbox: WallboxStrategicSnapshot;
};
...
function hasAlloc(
	...
	kind: string,
	nowMs: number,
	opts?: { currentOnly?: boolean; futureOnly?: boolean },
): boolean {
	for (const c of cells) {
		if (c.kind !== kind) continue;
		if (!(c.allocatedPowerW >= ON_W || c.allocatedEnergyKwh > 0.02)) continue;
		...
		if (current || futureOnly || b > nowMs) return true;
	}
	return false;
}

function batteryLabelDe(s: BatteryStrategicStatus): string {
	switch (s) {
		case "charge":
...
function wallboxLabelDe(s: WallboxStrategicStatus): string {
	switch (s) {
 ...
 */
export function deriveBatteryStrategicStatus(input: {
	plan: UnifiedDayPlan;
	socPct: number | null;
	...
	/** Wallbox-Immediate / bewusstes Battery-Hold. */
	batteryHold: boolean;
	...
	const hasCharge =
		hasAlloc(plan.allocations, "battery_charge", nowMs) ||
	...
	const nightInPlan =
		plan.reasonCodes.includes("battery_night_reserve") ||
	...
	let status: BatteryStrategicStatus;
	let reasonDe: string;
	...
	const nightTxt =
		night != null && night > 0 ? ` · Nachtreserve ~${night.toFixed(1).replace(".", ",")} kWh` : "";
	const summaryDe =
		status === "charge"
			? `${labelDe} · ${socTxt}`
			: status === "reserve_protected"
 ...
 */
export function deriveWallboxStrategicStatus(input: {
	plan: UnifiedDayPlan;
	connectedNow: boolean | null;
	requiredEnergyKwh: number | null;
	deadlineIso: string | null;
	hasHardFuturePresence: boolean;
	nowMs: number;
	...
	const need =
		input.requiredEnergyKwh != null && input.requiredEnergyKwh > 0.05
	...
	let status: WallboxStrategicStatus;
	let reasonDe: string;
...
export function buildAddonStrategicPlanSnapshot(input: {
	plan: UnifiedDayPlan;
	plannerInput: Pick<UnifiedDayPlannerInput, "battery" | "wallbox">;
	nowMs: number;
	generatedAtIso?: string;
	...
	const hasHardFuture = (wb?.presenceWindows ?? []).some(
		(w) =>
			(w.status === "available" || w.available === true) &&
