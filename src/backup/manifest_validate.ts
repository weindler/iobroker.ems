...
import type { ExportArchiveEntry, ExportManifest } from "./types";
...
export function assertUniqueArchivePaths(entries: ExportArchiveEntry[]): void {
 ...
 */
export function validateManifestPayloadConsistency(
	manifest: ExportManifest,
	payloadEntries: ExportArchiveEntry[],
): void {
	...
	for (const entry of payloadEntries) {
		...
		if (mf.sha256 !== sha256Buffer(buf)) {
			throw new Error(`sha256 mismatch for ${p}`);
		}
	}
}

export function extractManifestFromArchiveEntries(allEntries: ExportArchiveEntry[]): ExportManifest {
	const manifestEntry = allEntries.find((e) => e.path.replace(/\\/g, "/") === "manifest.json");
	if (!manifestEntry) {
		throw new Error("manifest.json missing from archive");
	}
	const text =
		typeof manifestEntry.content === "string" ? manifestEntry.content : manifestEntry.content.toString("utf8");
