 ...
 */
export async function ensureShadowEngineStates(host: StateHost): Promise<void> {
	await ensureChannel(host, "learning.shadow_engine", "EMS-Light Learning Shadow/Counterfactual-Engine");
	const defs: StateDef[] = [
		...
		numState(
			...
			"Shadow-Engine gestern real Netto-Kosten",
			"EUR",
		),
		numState(
		...
		numState(
			...
			"Shadow-Engine gestern ems_without_ai Netto-Kosten",
			"EUR",
		),
	];
