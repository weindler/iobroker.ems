...
function validDaySummaries(days: PriceDaySummary[]): PriceDaySummary[] {
  return days.filter(
    (d) => d.validHours >= MIN_VALID_HOURS_PER_DAY && d.avgPriceEur !== null,
  );
}
 ...
 */
export function robustWeightedMean(
...
function avgForWindow(
  validDays: PriceDaySummary[],
  maxDayOffset: number,
  doubleRecent = false,
): number | null {
  const prices = validDays
    ...
    .map((d) => ({
      value: d.avgPriceEur as number,
      weight: doubleRecent && d.dayOffset < RECENT_DOUBLE_WEIGHT_DAYS ? 2 : 1,
    }));
...
export function buildHourPatterns(
  ...
  const byHour = new Map<number, Array<{ value: number; weight: number }>>();
  const recentCutoff = Date.now() - RECENT_DOUBLE_WEIGHT_DAYS * 86_400_000;
...
export function computeConfidence(params: {
  ...
  const sampleRatio = Math.min(
    1,
    params.sampleDays / Math.max(1, params.lookbackDays),
  );
