...
class Ems extends utils.Adapter {
    ...
    onMessage(obj) {
                ...
                try {
                    ...
                    if (obj.callback) {
                        if (started.ok) {
                            ...
                            this.sendTo(obj.from, obj.command, { result: "error", error: started.error }, obj.callback);
                        }
                    }
                }
                catch (e) {
        ...
        if (obj.command === "aiOptimizeNow") {
            void (async () => {
                try {
                    ...
                    if (obj.callback) {
                        const result = outcome.ran && outcome.status === "ready"
                            ? "ok"
        ...
        if (obj.command === "aiDailyAnalystNow") {
            void (async () => {
                try {
                    const { runDailyAnalystFromAdminButton, asDailyAnalystAdapterHost } = await import("./ai/daily_analyst/index.js");
