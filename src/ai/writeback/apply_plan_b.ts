...
import { energyKwhFromPower } from "../../operator/daily_plan/slots";
...
function flexEntries(slot: DailyPlanSlot, prefix: string): DailyAllocationEntry[] {
	return slot.allocations.filter((a) => !a.mandatory && a.contributionId.startsWith(prefix));
}

function sumFlexW(slot: DailyPlanSlot, prefix: string): number {
...
function ensureFlexEntry(
	slot: DailyPlanSlot,
	prefix: string,
	template: DailyAllocationEntry | null,
): DailyAllocationEntry | null {
	const existing = flexEntries(slot, prefix);
	if (existing.length > 0) return existing[0]!;
	if (!template) return null;
	const created: DailyAllocationEntry = {
		...(JSON.parse(JSON.stringify(template)) as DailyAllocationEntry),
...
function applyNewPowerToFlex(
	...
	if (oldW <= 0) {
		...
		a.allocatedEnergyKwh = energyKwhFromPower(newW, slotMinutes);
		a.estimatedCostCt =
	...
	for (const a of entries) {
		...
		a.allocatedEnergyKwh = energyKwhFromPower(next, slotMinutes);
		a.estimatedCostCt =
			slot.gridPriceCtPerKwh !== null
				? Math.round(a.gridPowerW * (slotMinutes / 60) * slot.gridPriceCtPerKwh) / 1000
				: a.estimatedCostCt;
...
function recomputeSlotAggregates(slot: DailyPlanSlot): void {
	let flex = 0;
	let pv = 0;
	let grid = 0;
	let bat = 0;
...
function redistributeEligible(
	...
	return redistributeAddonAcrossSlots(
		...
		multipliers,
		inferMinPowerW(ownWPerSlot, runtimeId),
	);
}
...
export function applyAiPreferencesToDailyPlan(
	...
	const active = COMPARE_ELIGIBLE_GOVERNED_IDS.filter((id) => allowedAddonIds.includes(id));
	for (const id of active) {
		const prefix = contributionPrefixForCompare(id);
		const template =
