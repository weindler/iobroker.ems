...
describe("price learning math", () => {
  it("treats missing as not zero", () => {
    ...
    assert.equal(isValidPriceValue(-80, "ct_per_kwh"), true);
    assert.equal(isValidPriceValue(600, "ct_per_kwh"), true);
    assert.equal(isValidPriceValue(1_100, "ct_per_kwh"), false);
  });

  it("keeps extremes but limits their influence on the learned center", () => {
    const mean = robustWeightedMean([
      { value: -1, weight: 1 },
      { value: 0.2, weight: 1 },
      { value: 0.21, weight: 1 },
      { value: 0.22, weight: 1 },
    ...
    assert.ok(mean !== null && mean > 0.15 && mean < 0.3);
  });

  it("weights the most recent 90 days twice in the 24-month basis", () => {
    const days = [
      ...Array.from({ length: 90 }, (_, i) => daySummary(i, 24, 0.4)),
      ...Array.from({ length: 90 }, (_, i) => daySummary(i + 90, 24, 0.2)),
    ];
    const result = computePriceLearning([], days, 180, "ems_live_price");
    assert.ok(result.avgPrice24m !== null);
    assert.ok(result.avgPrice24m > 0.3);
  });
  ...
  it("computes averages volatility and confidence", () => {
    const days = Array.from({ length: 30 }, (_, i) =>
      daySummary(i, 20, 0.2 + (i % 5) * 0.01),
    );
    const samples = days.flatMap((d) =>
      Array.from({ length: 20 }, (_, h) =>
        sample(h, d.dayOffset, d.avgPriceEur ?? 0.2),
      ),
    );
  ...
  it("writes persist file", async () => {
    ...
    const raw = await fs.readFile(
      path.join(dir, "price_learning_v1.json"),
      "utf8",
    );
