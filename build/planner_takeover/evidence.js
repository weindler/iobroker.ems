...
exports.reconcileLoadedEvidence = exports.applyDualRunToEvidence = exports.sealEvidence = exports.computeEvidenceRevision = exports.emptyTakeoverEvidence = exports.policyFingerprint = void 0;
...
function policyFingerprint(policy) {
    return (0, node_crypto_1.createHash)("sha256").update(JSON.stringify((0, canonical_1.sortKeysDeep)(policy)), "utf8").digest("hex").slice(0, 16);
}
exports.policyFingerprint = policyFingerprint;
function emptyTakeoverEvidence(policy = constants_1.DEFAULT_TAKEOVER_READINESS_POLICY) {
    return {
        schemaVersion: constants_1.TAKEOVER_EVIDENCE_SCHEMA_VERSION,
        state: "not_evaluated",
        eligibleRuns: 0,
        matchedRuns: 0,
        mismatchedRuns: 0,
...
exports.emptyTakeoverEvidence = emptyTakeoverEvidence;
function computeEvidenceRevision(evidence) {
    return (0, node_crypto_1.createHash)("sha256").update(JSON.stringify((0, canonical_1.sortKeysDeep)(evidence)), "utf8").digest("hex");
}
exports.computeEvidenceRevision = computeEvidenceRevision;
...
function mapFailureBlockReason(errorCode) {
    if (!errorCode)
        return "worker_failed";
    if (errorCode.includes("timeout"))
        return "worker_timeout";
    if (errorCode.includes("crash") || errorCode.includes("exit"))
        return "worker_crash";
    if (errorCode.includes("shutdown") || errorCode.includes("stopping"))
        return "shutdown";
    if (errorCode.includes("authoritative"))
        return "authoritative_failed";
    return "worker_failed";
}
function recomputeState(evidence, policy, nowMs) {
    ...
    if (!seriesHealthy && evidence.failedRuns > policy.maxFailures && evidence.lastFailureAt) {
        return {
            state: "blocked",
            blockReason: evidence.lastBlockReason && evidence.lastBlockReason !== "evaluation_disabled"
    ...
    const freshOk = Number.isFinite(lastEligibleMs) && nowMs - lastEligibleMs <= policy.maxStaleEligibleMs;
    const missing = [];
    if (evidence.eligibleRuns < policy.minEligibleRuns)
        missing.push("insufficient_runs");
    if (evidence.consecutiveMatches < policy.minConsecutiveMatches)
        missing.push("insufficient_runs");
    if (!observationOk)
        missing.push("insufficient_observation_time");
    if (evidence.observedDistinctUtcDays < policy.minDistinctUtcDays)
        missing.push("insufficient_distinct_days");
    if (policy.requireSlotTransition && evidence.observedSlotTransitions < 1)
        missing.push("no_slot_transition");
    if (policy.requireDayTransition && evidence.observedDayTransitions < 1)
        missing.push("no_day_transition");
    if (!freshOk)
        missing.push("stale_eligible_run");
 ...
 */
function applyDualRunToEvidence(previous, event) {
    const policy = event.policy ?? constants_1.DEFAULT_TAKEOVER_READINESS_POLICY;
    ...
    let evidence = previous &&
        previous.schemaVersion === constants_1.TAKEOVER_EVIDENCE_SCHEMA_VERSION &&
        ...
        : {
            ...
            lastBlockReason: previous ? "policy_reset" : null,
            policyFingerprint: fp,
        };
    ...
    const day = (0, canonize_1.utcDayKey)(event.nowIso);
    if (diagnosticOnly) {
    ...
    const eligibleStatuses = ["matched", "mismatch", "validation_failed", "worker_failed", "authoritative_failed"];
    const isEligibleShape = eligibleStatuses.includes(event.compareStatus) &&
...
exports.applyDualRunToEvidence = applyDualRunToEvidence;
function trackTransitions(evidence, event, day) {
    ...
    if (prevStart && prevEnd && (prevStart !== nextStart || prevEnd !== nextEnd)) {
        ...
        if ((0, canonize_1.utcDayKey)(prevStart) !== (0, canonize_1.utcDayKey)(nextStart)) {
 ...
 */
function reconcileLoadedEvidence(raw, policy = constants_1.DEFAULT_TAKEOVER_READINESS_POLICY) {
    const fp = policyFingerprint(policy);
    ...
    const e = raw;
    if (e.schemaVersion !== constants_1.TAKEOVER_EVIDENCE_SCHEMA_VERSION) {
        return {
...
exports.reconcileLoadedEvidence = reconcileLoadedEvidence;
