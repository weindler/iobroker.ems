...
exports.applyDayToLearningState = void 0;
const learning_math_1 = require("./learning_math");
const TARGET_SAMPLES_DAILY_METRIC = 20;
function classificationToScore(q) {
    switch (q) {
        case "mandatory":
        case "necessary":
        case "reasonable":
...
function applyDayToLearningState(state, record, findings, nowIso = new Date().toISOString()) {
    if (state.lastProcessedDateKey && record.dateKey <= state.lastProcessedDateKey) {
        return state;
    }
    let next = { ...state };
    for (const f of findings) {
        if (f.insufficientData || f.notApplicable)
            continue;
        if (f.domain === "battery" && f.eventType === "battery_reserve_check") {
            const held = f.reasonCodes.includes("reserve_held");
            next = {
                ...next,
                batteryReserveAccuracyPct: (0, learning_math_1.updateLearningMetric)(next.batteryReserveAccuracyPct, held ? 100 : 0, nowIso, TARGET_SAMPLES_DAILY_METRIC),
            };
        }
        if (f.domain === "thermal" && f.reasonCodes.includes("daily_plan_price_timed")) {
            ...
            if (score != null) {
                next = {
                    ...next,
                    thermalPriceTimingScore: (0, learning_math_1.updateLearningMetric)(next.thermalPriceTimingScore, score, nowIso, TARGET_SAMPLES_DAILY_METRIC),
                };
            }
        }
        if (f.domain === "climate" && f.reasonCodes.includes("price_timed")) {
            ...
            if (score != null) {
                next = {
                    ...next,
                    climatePriceTimingScore: (0, learning_math_1.updateLearningMetric)(next.climatePriceTimingScore, score, nowIso, TARGET_SAMPLES_DAILY_METRIC),
                };
            }
        }
        if (f.domain === "ev" && f.eventType === "ev_readiness_check") {
            ...
            next = {
                ...next,
                evReadinessMetRatePct: (0, learning_math_1.updateLearningMetric)(next.evReadinessMetRatePct, met ? 100 : 0, nowIso, TARGET_SAMPLES_DAILY_METRIC),
            };
        }
    }
    ...
    if (pvScore?.value != null) {
        next = {
            ...next,
            pvUtilizationPct: (0, learning_math_1.updateLearningMetric)(next.pvUtilizationPct, pvScore.value, nowIso, TARGET_SAMPLES_DAILY_METRIC),
        };
    }
    ...
    if (priceScore?.value != null) {
        next = {
            ...next,
            priceEfficiencyScore: (0, learning_math_1.updateLearningMetric)(next.priceEfficiencyScore, priceScore.value, nowIso, TARGET_SAMPLES_DAILY_METRIC),
        };
    }
    next = { ...next, lastProcessedDateKey: record.dateKey, updatedAtIso: nowIso };
    return next;
}
exports.applyDayToLearningState = applyDayToLearningState;
