...
exports.estimateDehumidifyHours = exports.estimateCoolingHours = exports.outdoorDriveFactor = void 0;
function outdoorDriveFactor(outdoorMaxC, outdoorLikelyTempC) {
    if (outdoorMaxC === null || !Number.isFinite(outdoorMaxC))
        return 0;
    if (outdoorMaxC < outdoorLikelyTempC - 2)
        return 0;
    if (outdoorMaxC < outdoorLikelyTempC)
        return 0.35;
    if (outdoorMaxC < outdoorLikelyTempC + 3)
        return 0.55;
    if (outdoorMaxC < outdoorLikelyTempC + 6)
        return 0.75;
    return 0.95;
}
exports.outdoorDriveFactor = outdoorDriveFactor;
function estimateCoolingHours(input) {
    ...
    const learned = input.learnedHours !== null && input.learnedHours > 0 ? input.learnedHours : null;
    ...
    if (factor <= 0 && (room === null || room <= input.offTempC)) {
        return {
            likelyActive: false,
            expectedHours: 0,
            reasonDe: input.outdoorMaxC !== null
...
exports.estimateCoolingHours = estimateCoolingHours;
function estimateDehumidifyHours(input) {
    ...
    const learned = input.learnedHours !== null && input.learnedHours > 0 ? input.learnedHours * 0.5 : null;
    ...
    return {
        likelyActive: false,
        expectedHours: 0,
        reasonDe: humidity !== null
            ? `Feuchte ${humidity.toFixed(0)} % unter ${input.maxHumidityPct} %`
            : "Kein Entfeuchtungsbedarf",
    };
}
exports.estimateDehumidifyHours = estimateDehumidifyHours;
function round2(n) {
