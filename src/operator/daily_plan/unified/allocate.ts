...
function buildBatteryTrajectory(
	...
	for (const s of slots) {
		...
		for (const a of allocations) {
			...
			if (a.kind === "battery_charge") chargeAc += a.allocatedEnergyKwh;
			if (a.kind === "battery_discharge") dischargeAc += a.allocatedEnergyKwh;
			/*
		 ...
		 */
		const futurePassiveAvailable =
			input.battery.passiveBatteryEnergyForecastAvailable ??
			input.battery.passiveBatteryEnergyAvailable;
		const passiveAvailable =
			Number.isFinite(nowMs) && s.startMs > nowMs
		...
		const storedDischarge = Math.min(
			dischargeAc / Math.max(effD, 0.1),
			Math.max(0, soc - reserveKwh),
		);
