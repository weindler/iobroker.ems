...
export type NotificationCandidateKind =
	| "vehicle_grid_charge_recommended"
	| "vehicle_goal_at_risk"
	| "forecast_collapse"
	| "unexpected_surplus"
	| "goal_missed"
	| "planning_data_missing";

export type NotificationCandidate = {
	schemaVersion: 1;
	kind: NotificationCandidateKind;
	/** Stabile Dedup-ID: kind|date|subject — gleiche Sache mehrfach replanned → gleicher Key. */
	dedupKey: string;
	date: string;
	createdAtIso: string;
...
export function buildNotificationCandidates(input: {
	plan: UnifiedDayPlan;
	date: string;
	nowIso: string;
	previousExpectedPvKwh?: number | null;
	evaluation?: DayEvaluationRecord | null;
	...
	if (gridKwh >= MATERIAL_GRID_CHARGE_KWH && eco) {
		const savings =
			eco.savingsVsAlternativeCt !== null && eco.economicsCompleteness !== "unknown"
	...
	if (
		plan.reasonCodes.includes("battery_telemetry_missing") ||
	...
	for (const c of out) {
		if (!byKey.has(c.dedupKey)) byKey.set(c.dedupKey, c);
	}
	void MATERIAL_SAVINGS_CT;
	return [...byKey.values()];
}

function sumKind(plan: UnifiedDayPlan, kind: string): number {
	return Math.round(
...
export function mergeNotificationCandidates(
	existing: NotificationCandidate[],
	incoming: NotificationCandidate[],
): NotificationCandidate[] {
	const map = new Map<string, NotificationCandidate>();
	for (const c of existing) map.set(c.dedupKey, c);
	for (const c of incoming) {
		if (!map.has(c.dedupKey)) map.set(c.dedupKey, c);
	}
	return [...map.values()];
}
