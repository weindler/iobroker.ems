"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.ensureWallboxEvFoundationStates = exports.WALLBOX_EV_FOUNDATION_BALLAST_SUFFIXES = exports.WALLBOX_EV_FOUNDATION_PUBLIC_STATE_IDS = exports.WALLBOX_EV_FOUNDATION_STATES = void 0;
const tree_paths_1 = require("../../../tree_paths");
const state_util_1 = require("../../../ems_light/state_util");
const EV_FOUNDATION_BASE = `${(0, tree_paths_1.addonStatusBase)("wallbox")}.ev_foundation`;
exports.WALLBOX_EV_FOUNDATION_STATES = {
    evccReachable: `${EV_FOUNDATION_BASE}.evcc_reachable`,
    vehicleConnected: `${EV_FOUNDATION_BASE}.vehicle_connected`,
    charging: `${EV_FOUNDATION_BASE}.charging`,
    chargePowerW: `${EV_FOUNDATION_BASE}.charge_power_w`,
    evccMode: `${EV_FOUNDATION_BASE}.evcc_mode`,
    phasesConfigured: `${EV_FOUNDATION_BASE}.phases_configured`,
    phasesActive: `${EV_FOUNDATION_BASE}.phases_active`,
    vehicleSocPct: `${EV_FOUNDATION_BASE}.vehicle_soc_pct`,
    vehicleSocQuality: `${EV_FOUNDATION_BASE}.vehicle_soc_quality`,
    capabilitiesJson: `${EV_FOUNDATION_BASE}.capabilities_json`,
    externalControlActive: `${EV_FOUNDATION_BASE}.external_control_active`,
    externalControlType: `${EV_FOUNDATION_BASE}.external_control_type`,
    externalControlConfigured: `${EV_FOUNDATION_BASE}.external_control_configured`,
    gridRewardsActive: `${EV_FOUNDATION_BASE}.grid_rewards_active`,
    smartChargingActive: `${EV_FOUNDATION_BASE}.smart_charging_active`,
    externalSourceQuality: `${EV_FOUNDATION_BASE}.external_source_quality`,
    externalSourceUpdatedAt: `${EV_FOUNDATION_BASE}.external_source_updated_at`,
    externalSmartPlanMappingConfigured: `${EV_FOUNDATION_BASE}.external_smart_plan_mapping_configured`,
    externalSmartPlanParseable: `${EV_FOUNDATION_BASE}.external_smart_plan_parseable`,
    externalSmartPlanAvailable: `${EV_FOUNDATION_BASE}.external_smart_plan_available`,
    externalSmartPlanSlotCount: `${EV_FOUNDATION_BASE}.external_smart_plan_slot_count`,
    externalSmartPlanNextStart: `${EV_FOUNDATION_BASE}.external_smart_plan_next_start`,
    externalSmartPlanLastEnd: `${EV_FOUNDATION_BASE}.external_smart_plan_last_end`,
    externalPlanRemainingEnergyKwh: `${EV_FOUNDATION_BASE}.external_plan_remaining_energy_kwh`,
    externalPlanRemainingMinutes: `${EV_FOUNDATION_BASE}.external_plan_remaining_minutes`,
    externalPlanDeadlineUsed: `${EV_FOUNDATION_BASE}.external_plan_deadline_used`,
    externalSmartPlanJson: `${EV_FOUNDATION_BASE}.external_smart_plan_json`,
    externalRawDiagnosticsJson: `${EV_FOUNDATION_BASE}.external_raw_diagnostics_json`,
    departureMinSocConfigured: `${EV_FOUNDATION_BASE}.departure_min_soc_configured`,
    externalMinSocPct: `${EV_FOUNDATION_BASE}.external_min_soc_pct`,
    externalMinSocQuality: `${EV_FOUNDATION_BASE}.external_min_soc_quality`,
    vehicleModelSource: `${EV_FOUNDATION_BASE}.vehicle_model_source`,
    vehicleModelReady: `${EV_FOUNDATION_BASE}.vehicle_model_ready`,
    controlContractModel: `${EV_FOUNDATION_BASE}.control_contract_model`,
    evccControlContractReady: `${EV_FOUNDATION_BASE}.evcc_control_contract_ready`,
    legacyDirectControlPresent: `${EV_FOUNDATION_BASE}.legacy_direct_control_present`,
    evccModeControlVariant: `${EV_FOUNDATION_BASE}.evcc_mode_control_variant`,
    evccModeFeedbackState: `${EV_FOUNDATION_BASE}.evcc_mode_feedback_state`,
    evccModeButtonsReady: `${EV_FOUNDATION_BASE}.evcc_mode_buttons_ready`,
    evccModeOffTargetReady: `${EV_FOUNDATION_BASE}.evcc_mode_off_target_ready`,
    evccModePvTargetReady: `${EV_FOUNDATION_BASE}.evcc_mode_pv_target_ready`,
    evccModeMinTargetReady: `${EV_FOUNDATION_BASE}.evcc_mode_min_target_ready`,
    evccModeNowTargetReady: `${EV_FOUNDATION_BASE}.evcc_mode_now_target_ready`,
    preparedEvState: `${EV_FOUNDATION_BASE}.prepared_ev_state`,
    takeoverReason: `${EV_FOUNDATION_BASE}.takeover_reason`,
    externalAuthorityState: `${EV_FOUNDATION_BASE}.external_authority_state`,
    energyToTargetKwh: `${EV_FOUNDATION_BASE}.energy_to_target_kwh`,
    energyToDepartureMinimumKwh: `${EV_FOUNDATION_BASE}.energy_to_departure_minimum_kwh`,
    requiredChargingMinutes: `${EV_FOUNDATION_BASE}.required_charging_minutes`,
    latestRequiredStart: `${EV_FOUNDATION_BASE}.latest_required_start`,
    deadlineRisk: `${EV_FOUNDATION_BASE}.deadline_risk`,
    externalPlanExpectedSocGainPct: `${EV_FOUNDATION_BASE}.external_plan_expected_soc_gain_pct`,
    externalPlanExpectedFinalSocPct: `${EV_FOUNDATION_BASE}.external_plan_expected_final_soc_pct`,
    externalPlanCoversTarget: `${EV_FOUNDATION_BASE}.external_plan_covers_target`,
    externalPlanCoversDepartureMinimum: `${EV_FOUNDATION_BASE}.external_plan_covers_departure_minimum`,
    remainingFeasibleEnergyKwh: `${EV_FOUNDATION_BASE}.remaining_feasible_energy_kwh`,
    remainingCheapEnergyKwh: `${EV_FOUNDATION_BASE}.remaining_cheap_energy_kwh`,
    economicWindowLossRisk: `${EV_FOUNDATION_BASE}.economic_window_loss_risk`,
    takeoverRecommended: `${EV_FOUNDATION_BASE}.takeover_recommended`,
    takeoverRequired: `${EV_FOUNDATION_BASE}.takeover_required`,
    takeoverSeverity: `${EV_FOUNDATION_BASE}.takeover_severity`,
    recommendedEvState: `${EV_FOUNDATION_BASE}.recommended_ev_state`,
    takeoverDecisionJson: `${EV_FOUNDATION_BASE}.takeover_decision_json`,
    dataQuality: `${EV_FOUNDATION_BASE}.data_quality`,
    modelJson: `${EV_FOUNDATION_BASE}.model_json`,
    updatedAt: `${EV_FOUNDATION_BASE}.updated_at`,
    evPlannerParticipating: `${EV_FOUNDATION_BASE}.ev_planner_participating`,
    evPlannerRole: `${EV_FOUNDATION_BASE}.ev_planner_role`,
    evManagementMode: `${EV_FOUNDATION_BASE}.ev_management_mode`,
    evHardEnergyKwh: `${EV_FOUNDATION_BASE}.ev_hard_energy_kwh`,
    evTargetEnergyKwh: `${EV_FOUNDATION_BASE}.ev_target_energy_kwh`,
    evAcEnergyRequiredKwh: `${EV_FOUNDATION_BASE}.ev_ac_energy_required_kwh`,
    evPlannedEnergyKwh: `${EV_FOUNDATION_BASE}.ev_planned_energy_kwh`,
    evUnplannedEnergyKwh: `${EV_FOUNDATION_BASE}.ev_unplanned_energy_kwh`,
    evPlannedCostEur: `${EV_FOUNDATION_BASE}.ev_planned_cost_eur`,
    evPlannedPvEnergyKwh: `${EV_FOUNDATION_BASE}.ev_planned_pv_energy_kwh`,
    evPlannedGridEnergyKwh: `${EV_FOUNDATION_BASE}.ev_planned_grid_energy_kwh`,
    evPlannedFirstStart: `${EV_FOUNDATION_BASE}.ev_planned_first_start`,
    evPlannedLastEnd: `${EV_FOUNDATION_BASE}.ev_planned_last_end`,
    evPlanQuality: `${EV_FOUNDATION_BASE}.ev_plan_quality`,
    evPlanJson: `${EV_FOUNDATION_BASE}.ev_plan_json`,
    evExecutionEnabled: `${EV_FOUNDATION_BASE}.ev_execution_enabled`,
    evExecutionAuthority: `${EV_FOUNDATION_BASE}.ev_execution_authority`,
    evExecutionReady: `${EV_FOUNDATION_BASE}.ev_execution_ready`,
    evExecutionBlockReason: `${EV_FOUNDATION_BASE}.ev_execution_block_reason`,
    evExecutionDesiredMode: `${EV_FOUNDATION_BASE}.ev_execution_desired_mode`,
    evExecutionDesiredReason: `${EV_FOUNDATION_BASE}.ev_execution_desired_reason`,
    evExecutionActualMode: `${EV_FOUNDATION_BASE}.ev_execution_actual_mode`,
    evExecutionSourceFresh: `${EV_FOUNDATION_BASE}.ev_execution_source_fresh`,
    evExecutionOwnership: `${EV_FOUNDATION_BASE}.ev_execution_ownership`,
    evExecutionOwnedMode: `${EV_FOUNDATION_BASE}.ev_execution_owned_mode`,
    evExecutionOwnedSince: `${EV_FOUNDATION_BASE}.ev_execution_owned_since`,
    evExecutionReleaseReason: `${EV_FOUNDATION_BASE}.ev_execution_release_reason`,
    evExecutionPendingMode: `${EV_FOUNDATION_BASE}.ev_execution_pending_mode`,
    evExecutionPendingSince: `${EV_FOUNDATION_BASE}.ev_execution_pending_since`,
    evExecutionLastCommand: `${EV_FOUNDATION_BASE}.ev_execution_last_command`,
    evExecutionLastCommandAt: `${EV_FOUNDATION_BASE}.ev_execution_last_command_at`,
    evExecutionLastFeedbackAt: `${EV_FOUNDATION_BASE}.ev_execution_last_feedback_at`,
    evExecutionRetryCount: `${EV_FOUNDATION_BASE}.ev_execution_retry_count`,
    evExecutionLastResult: `${EV_FOUNDATION_BASE}.ev_execution_last_result`,
    evExecutionFailsafeReason: `${EV_FOUNDATION_BASE}.ev_execution_failsafe_reason`,
    evExecutionPhase: `${EV_FOUNDATION_BASE}.ev_execution_phase`,
    evExecutionExplain: `${EV_FOUNDATION_BASE}.ev_execution_explain`,
    tibberNowHandoffEnabled: `${EV_FOUNDATION_BASE}.tibber_now_handoff_enabled`,
    tibberNowHandoffStatus: `${EV_FOUNDATION_BASE}.tibber_now_handoff_status`,
    tibberNowHandoffDueAt: `${EV_FOUNDATION_BASE}.tibber_now_handoff_due_at`,
    evExecutionLiveTestArmed: `${EV_FOUNDATION_BASE}.ev_execution_live_test_armed`,
    evExecutionLiveTestDisarm: `${EV_FOUNDATION_BASE}.ev_execution_live_test_disarm`,
    evExecutionLiveTestConsumed: `${EV_FOUNDATION_BASE}.ev_execution_live_test_consumed`,
    evExecutionLiveTestArmedAt: `${EV_FOUNDATION_BASE}.ev_execution_live_test_armed_at`,
    evExecutionLiveTestConsumedAt: `${EV_FOUNDATION_BASE}.ev_execution_live_test_consumed_at`,
    evExecutionLiveTestCommand: `${EV_FOUNDATION_BASE}.ev_execution_live_test_command`,
    evExecutionLiveTestResult: `${EV_FOUNDATION_BASE}.ev_execution_live_test_result`,
    evExecutionLiveTestBlockReason: `${EV_FOUNDATION_BASE}.ev_execution_live_test_block_reason`,
};
/** Öffentliche EV-Foundation: Authority, Smart-Plan-Inputs, kompakte Execution, Live-Test. */
exports.WALLBOX_EV_FOUNDATION_PUBLIC_STATE_IDS = new Set([
    exports.WALLBOX_EV_FOUNDATION_STATES.externalSmartPlanJson,
    exports.WALLBOX_EV_FOUNDATION_STATES.externalMinSocPct,
    exports.WALLBOX_EV_FOUNDATION_STATES.externalAuthorityState,
    exports.WALLBOX_EV_FOUNDATION_STATES.gridRewardsActive,
    exports.WALLBOX_EV_FOUNDATION_STATES.takeoverSeverity,
    exports.WALLBOX_EV_FOUNDATION_STATES.preparedEvState,
    exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionEnabled,
    exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionAuthority,
    exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionReady,
    exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionBlockReason,
    exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionDesiredMode,
    exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionExplain,
    exports.WALLBOX_EV_FOUNDATION_STATES.tibberNowHandoffEnabled,
    exports.WALLBOX_EV_FOUNDATION_STATES.tibberNowHandoffStatus,
    exports.WALLBOX_EV_FOUNDATION_STATES.tibberNowHandoffDueAt,
    exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLiveTestArmed,
    exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLiveTestDisarm,
    exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLiveTestConsumed,
    exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLiveTestResult,
    exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLiveTestBlockReason,
]);
exports.WALLBOX_EV_FOUNDATION_BALLAST_SUFFIXES = Object.values(exports.WALLBOX_EV_FOUNDATION_STATES)
    .filter((id) => !exports.WALLBOX_EV_FOUNDATION_PUBLIC_STATE_IDS.has(id))
    .map((id) => id.slice(EV_FOUNDATION_BASE.length + 1));
function boolState(id, name, write = false, role = "state") {
    return {
        id,
        common: { name, type: "boolean", role, read: true, write, def: false },
    };
}
function numState(id, name, unit) {
    return {
        id,
        common: {
            name,
            type: "number",
            role: unit === "W" ? "value.power" : "value",
            unit,
            read: true,
            write: false,
        },
    };
}
function strState(id, name, role = "text") {
    return {
        id,
        common: { name, type: "string", role, read: true, write: false },
    };
}
async function ensureWallboxEvFoundationStates(host) {
    await (0, state_util_1.ensureChannel)(host, EV_FOUNDATION_BASE, "EV-Fundament (Diagnose)");
    await (0, state_util_1.ensureStates)(host, [
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.evccReachable, "EVCC erreichbar"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.vehicleConnected, "Fahrzeug verbunden"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.charging, "Fahrzeug lädt"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.chargePowerW, "Reale Ladeleistung", "W"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evccMode, "Aktueller EVCC-Modus"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.phasesConfigured, "Konfigurierte Phasen"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.phasesActive, "Aktive Phasen"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.vehicleSocPct, "Fahrzeug-SOC", "%"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.vehicleSocQuality, "Fahrzeug-SOC Qualität"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.capabilitiesJson, "EV Capabilities (JSON)", "json"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.externalControlConfigured, "Externe Steuerung konfiguriert"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.externalControlActive, "Externe Steuerung aktiv"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.externalControlType, "Externer Steuerungstyp"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.gridRewardsActive, "Grid Rewards aktiv"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.smartChargingActive, "Smart Charging aktiv"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.externalSourceQuality, "Externe Quellenqualität"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.externalSourceUpdatedAt, "Externe Quelle zuletzt", "date"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.externalSmartPlanMappingConfigured, "Smart-Plan-Mapping konfiguriert"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.externalSmartPlanParseable, "Smart-Plan parsebar"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.externalSmartPlanAvailable, "Externer Smart-Plan verfügbar"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.externalSmartPlanSlotCount, "Smart-Plan gültige Slots"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.externalSmartPlanNextStart, "Nächster Smart-Plan-Start", "date"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.externalSmartPlanLastEnd, "Letztes Smart-Plan-Ende", "date"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.externalPlanRemainingEnergyKwh, "Verbleibende externe Planenergie", "kWh"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.externalPlanRemainingMinutes, "Verbleibende Planminuten", "min"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.externalPlanDeadlineUsed, "Departure-Deadline angewandt"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.externalSmartPlanJson, "Smart-Plan Slots (JSON)", "json"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.externalRawDiagnosticsJson, "Externe Rohdiagnose (JSON)", "json"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.departureMinSocConfigured, "Mindest-SOC zur Abfahrt konfiguriert"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.externalMinSocPct, "Externer Smart-Charging-Mindest-SOC", "%"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.externalMinSocQuality, "Externer Mindest-SOC Qualität"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.vehicleModelSource, "Fahrzeugmodell-Quelle"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.vehicleModelReady, "Fahrzeugmodell bereit"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.controlContractModel, "Control-Contract-Modell"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.evccControlContractReady, "EVCC-Control-Contract bereit"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.legacyDirectControlPresent, "Legacy-Direktsteuerung vorhanden"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evccModeControlVariant, "EVCC Mode-Control-Variante"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evccModeFeedbackState, "EVCC Mode-Feedback-State"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.evccModeButtonsReady, "EVCC Mode-Buttons bereit"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.evccModeOffTargetReady, "EVCC control.off gemappt"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.evccModePvTargetReady, "EVCC control.pv gemappt"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.evccModeMinTargetReady, "EVCC control.min gemappt"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.evccModeNowTargetReady, "EVCC control.now gemappt"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.preparedEvState, "Vorbereiteter EV-Zustand"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.takeoverReason, "Takeover-Grund (vorbereitet)"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.externalAuthorityState, "Externe Ladehoheit"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.energyToTargetKwh, "Energie bis Ziel-SOC", "kWh"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.energyToDepartureMinimumKwh, "Energie bis Abfahrts-Mindest-SOC", "kWh"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.requiredChargingMinutes, "Erforderliche Ladezeit", "min"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.latestRequiredStart, "Spätester erforderlicher Start", "date"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.deadlineRisk, "Deadline-Risiko"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.externalPlanExpectedSocGainPct, "Erwarteter SOC-Gewinn durch externen Plan", "%"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.externalPlanExpectedFinalSocPct, "Erwarteter End-SOC durch externen Plan", "%"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.externalPlanCoversTarget, "Externer Plan deckt Ziel-SOC"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.externalPlanCoversDepartureMinimum, "Externer Plan deckt Abfahrts-Minimum"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.remainingFeasibleEnergyKwh, "Physikalisch noch ladbare Energie", "kWh"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.remainingCheapEnergyKwh, "Energie in günstigen Restfenstern", "kWh"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.economicWindowLossRisk, "Risiko verlorener günstiger Fenster"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.takeoverRecommended, "EMS-Takeover empfohlen (Diagnose)"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.takeoverRequired, "EMS-Takeover erforderlich (Diagnose)"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.takeoverSeverity, "Takeover-Schwere (Diagnose)"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.recommendedEvState, "Empfohlener EV-Zustand (Diagnose)"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.takeoverDecisionJson, "Takeover-Entscheidung (JSON)", "json"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.dataQuality, "EV-Datenqualität"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.modelJson, "EV-Datenmodell V1 (JSON)", "json"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.updatedAt, "EV-Fundament zuletzt gelesen", "date"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.evPlannerParticipating, "EV-Planner nimmt teil"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evPlannerRole, "EV-Planner-Rolle"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evManagementMode, "EV-Management-Modus"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.evHardEnergyKwh, "EV Hard-Energie", "kWh"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.evTargetEnergyKwh, "EV Ziel-Energie", "kWh"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.evAcEnergyRequiredKwh, "EV AC-Bedarf", "kWh"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.evPlannedEnergyKwh, "EV geplante Energie", "kWh"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.evUnplannedEnergyKwh, "EV ungeplante Energie", "kWh"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.evPlannedCostEur, "EV geplante Kosten", "EUR"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.evPlannedPvEnergyKwh, "EV geplante PV-Energie", "kWh"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.evPlannedGridEnergyKwh, "EV geplante Netzenergie", "kWh"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evPlannedFirstStart, "EV geplanter Start", "date"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evPlannedLastEnd, "EV geplantes Ende", "date"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evPlanQuality, "EV-Planqualität"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evPlanJson, "EV-Plan (JSON)", "json"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionEnabled, "EV-Execution Feature-Gate"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionAuthority, "EV-Execution-Hoheit"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionReady, "EV-Execution bereit"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionBlockReason, "EV-Execution Sperrgrund"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionDesiredMode, "EV-Execution Desired-Modus"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionDesiredReason, "EV-Execution Desired-Grund"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionActualMode, "EV-Execution Ist-Modus"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionSourceFresh, "EVCC-Quelle frisch"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionOwnership, "EV-Execution Ownership"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionOwnedMode, "EV-Execution besessener Modus"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionOwnedSince, "EV-Execution Ownership seit", "date"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionReleaseReason, "EV-Execution Release-Grund"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionPendingMode, "EV-Execution Pending-Modus"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionPendingSince, "EV-Execution Pending seit", "date"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLastCommand, "EV-Execution letzter Befehl"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLastCommandAt, "EV-Execution letzter Befehl um", "date"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLastFeedbackAt, "EV-Execution letztes Feedback um", "date"),
        numState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionRetryCount, "EV-Execution Retry-Zähler"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLastResult, "EV-Execution letztes Ergebnis"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionFailsafeReason, "EV-Execution Fail-Safe"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionPhase, "EV-Execution Phase"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionExplain, "EV-Execution Explain"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.tibberNowHandoffEnabled, "Tibber-NOW-Übergabe freigegeben"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.tibberNowHandoffStatus, "Tibber-NOW-Übergabe Status"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.tibberNowHandoffDueAt, "Tibber-NOW-Übergabe fällig um", "date"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLiveTestArmed, "EV Phase-5B Live-Test armed (One-Shot)", true, "switch"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLiveTestDisarm, "EV Phase-5B Live-Test abbrechen", true, "button"),
        boolState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLiveTestConsumed, "EV Phase-5B Live-Test verbraucht"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLiveTestArmedAt, "EV Phase-5B Live-Test armed um", "date"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLiveTestConsumedAt, "EV Phase-5B Live-Test verbraucht um", "date"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLiveTestCommand, "EV Phase-5B Live-Test Command"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLiveTestResult, "EV Phase-5B Live-Test Ergebnis"),
        strState(exports.WALLBOX_EV_FOUNDATION_STATES.evExecutionLiveTestBlockReason, "EV Phase-5B Live-Test Sperrgrund"),
    ].filter((d) => exports.WALLBOX_EV_FOUNDATION_PUBLIC_STATE_IDS.has(d.id)));
}
exports.ensureWallboxEvFoundationStates = ensureWallboxEvFoundationStates;
