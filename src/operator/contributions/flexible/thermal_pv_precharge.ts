...
export type ThermalPvPrechargeMode = "eco" | "balanced" | "comfort" | "forced" | "off" | string;

export type ThermalPvPrechargeInput = {
	now: Date;
	bufferTempC: number;
	planningMinTempC: number;
	planningMaxTempC: number;
	/** Bereits nach Forecast + Nachtbrücke (Mindestbedarf). */
	baseTargetTempC: number;
	coolingRateCPerHAvg: number | null;
	/** Reichweiten-Info; null = unbekannt — Vorladung trotzdem möglich. */
	estimatedEmptyAtIso: string | null;
	/** Nächster sinnvoller PV-Heizzeitpunkt (ISO). */
	...
	/** Hartes Fahrzeugziel im Horizont (kWh); >0 → Extra-Vorladung zurückstellen. */
	vehicleUrgentEnergyKwh: number | null;
	exportTariffCtPerKwh: number | null;
	/** Aktueller/typischer Importpreis (ct); hoch → thermisch speichern attraktiver. */
	importTariffCtPerKwh?: number | null;
	/**
	 * Später erwarteter elektrischer Flexbedarf (Klima/Fahrzeug), der von früher
	 * gespeicherter Wärme entlastet werden kann — nur Hinweis, keine Fake-0.
	 */
	futureElectricalFlexHintKwh?: number | null;
	globalMode: ThermalPvPrechargeMode;
};

export type ThermalPvPrechargeResult = {
	active: boolean;
	targetTempC: number;
	prechargeExtraK: number;
	reasonDe: string;
};
...
function modeAggressiveness(mode: ThermalPvPrechargeMode): number {
	if (mode === "eco") return 0.55;
	if (mode === "comfort" || mode === "forced") return 1.0;
 ...
 */
export function resolveThermalPvPrecharge(input: ThermalPvPrechargeInput): ThermalPvPrechargeResult {
	...
	const vehicleUrgent =
		input.vehicleUrgentEnergyKwh != null && input.vehicleUrgentEnergyKwh > 0.5;
	if (vehicleUrgent) {
		return {
			active: false,
			targetTempC: base,
			prechargeExtraK: 0,
			reasonDe:
				"Fahrzeugziel aktiv — thermische Extra-Vorladung zurückgestellt (PV für elektrische Flexibilität).",
		};
	}
	...
	if (
		...
		if (tempAtPv >= input.planningMinTempC - 0.05) {
			const emptyAfterPv =
				!Number.isFinite(emptyMs) || emptyMs >= nextPvMs - 60_000;
	...
	if (batterySated) fractionTowardMax += 0.4;
	if (batteryCompetes) fractionTowardMax *= 0.4;

	if (surplus != null && surplus >= 8) fractionTowardMax += 0.18;
	else if (surplus != null && surplus >= 5) fractionTowardMax += 0.08;
	...
	if (hasEmptyAt) {
		...
		if (hoursToEmpty < 8) fractionTowardMax += 0.12;
		else if (hoursToEmpty < 14) fractionTowardMax += 0.06;
	}
	...
	const span = max - base;
	let extra = round3(span * fractionTowardMax);
	...
	return {
		active: true,
		targetTempC: target,
		prechargeExtraK: round3(target - base),
		reasonDe: parts.join("; ") + ".",
	};
}
