...
exports.climateDemandDigest = exports.climateWindowEndMs = exports.estimateClimateUnitDemand = exports.hourlyPointsInWindow = void 0;
...
function modeDemand(hours, reasonDe, extra = {}) {
...
function hourlyPointsInWindow(points, fromMs, toMs) {
    if (!(toMs > fromMs) || !Array.isArray(points))
        return [];
    return points
        .filter((p) => {
...
exports.hourlyPointsInWindow = hourlyPointsInWindow;
function hourlyCoverage(points, fromMs, toMs) {
    const span = toMs - fromMs;
    if (!(span > 0))
        return 0;
    let covered = 0;
    for (const p of points) {
        if (p.outdoorTempC == null || !Number.isFinite(p.outdoorTempC))
            continue;
...
function futureOutdoorMaxC(points, fromMs, toMs) {
    let max = null;
...
function futureOutdoorMinC(points, fromMs, toMs) {
    let min = null;
    for (const p of hourlyPointsInWindow(points, fromMs, toMs)) {
        if (p.outdoorTempC == null || !Number.isFinite(p.outdoorTempC))
            continue;
...
function warmHoursInWindow(points, fromMs, toMs, thresholdC) {
    let sec = 0;
    for (const p of hourlyPointsInWindow(points, fromMs, toMs)) {
        if (p.outdoorTempC == null || p.outdoorTempC < thresholdC)
            continue;
        const a = Math.max(fromMs, Date.parse(p.startIso));
        const b = Math.min(toMs, Date.parse(p.endIso));
        if (b > a)
...
function coldHoursInWindow(points, fromMs, toMs, thresholdC) {
    let sec = 0;
    for (const p of hourlyPointsInWindow(points, fromMs, toMs)) {
        if (p.outdoorTempC == null || p.outdoorTempC > thresholdC)
            continue;
...
function statUsable(stat) {
    return !!stat && stat.usable === true && stat.rate != null && Number.isFinite(stat.rate);
}
function pickPassiveRateKPerH(model, roomC, outdoorC) {
    const p = model.passive;
    if (!statUsable(p) || p.rate == null)
        return null;
    if (outdoorC != null && Number.isFinite(outdoorC)) {
        if (outdoorC > roomC + 0.5 && p.warmingRateKPerH != null)
            return p.warmingRateKPerH;
        if (outdoorC < roomC - 0.5 && p.coolingRateKPerH != null)
            return p.coolingRateKPerH;
    }
    return p.rate;
}
...
function simulateRoomTemp(input) {
    ...
    let peak = room;
    let low = room;
    let crossingAtMs = null;
    const crossed = (v) => input.crossWhen === "above" ? v >= input.thresholdC : v <= input.thresholdC;
    if (crossed(room))
        crossingAtMs = input.fromMs;
...
function capWithLearnedHours(hours, remaining, learnedHours) {
...
function hoursFromDeltaK(deltaK, rateKPerH, remaining, learnedHours) {
    const absRate = Math.abs(rateKPerH);
    if (!(absRate > 0.05) || !(deltaK > 0))
        return 0;
    return capWithLearnedHours(deltaK / absRate, remaining, learnedHours);
}
function legacyCooling(input) {
...
function legacyDry(input, dryConfigured) {
...
function bootstrapCooling(input, fromMs, toMs) {
    const room = input.roomTempC;
    const onC = input.unit.onTempC;
    const offC = input.unit.offTempC;
    if (room == null)
        return noneMode("Raumtemperatur fehlt.");
    if (!(0, config_1.acModeCommandEnabled)(input.unit.modeWhenCooling)) {
    ...
    if (room <= offC) {
        return noneMode(`Raum ${room.toFixed(1)} °C ≤ ${offC} °C — kein Cooling-Budget ohne Raummodell`);
    }
    /* off < room < on: nähert sich der Grenze */
    const outdoorNowHot = input.outdoorTempC != null && input.outdoorTempC >= input.outdoorLikelyTempC;
    const outdoorUnfavorable = (futureMax != null && futureMax >= input.outdoorLikelyTempC) ||
        nWarm >= 0.25 ||
        outdoorNowHot;
    if (!outdoorUnfavorable) {
        return noneMode(`Raum ${room.toFixed(1)} °C unter ${onC} °C, Stundenforecast unkritisch`);
    }
    const factor = (0, climate_energy_1.outdoorDriveFactor)(futureMax ?? input.outdoorTempC, input.outdoorLikelyTempC);
    const hours = capWithLearnedHours(remaining * factor * BOOTSTRAP_PREEMPT_FRACTION, remaining, null);
    return modeDemand(hours, `Raum ${room.toFixed(1)} °C nähert sich ${onC} °C, Forecast bis ${futureMax?.toFixed(1) ?? "?"} °C — vorsichtiges Pre-Cooling`, { predictedPeak: futureMax });
}
function bootstrapHeating(input, fromMs, toMs) {
    if (!(0, config_1.acModeCommandEnabled)(input.unit.modeWhenHeating)) {
    ...
    const outdoorUnfavorable = (futureMin != null && futureMin <= setC) || nCold >= 0.25 || outdoorNowCold;
    if (!outdoorUnfavorable) {
    ...
    return modeDemand(hours, `Raum ${room.toFixed(1)} °C nähert sich Heizsoll ${setC} °C, Forecast bis ${futureMin?.toFixed(1) ?? "?"} °C — vorsichtiges Pre-Heating`, { predictedLow: futureMin });
}
function bootstrapDry(input) {
    const dryConfigured = (0, config_1.acModeCommandEnabled)(input.unit.modeWhenDehumidify) && input.unit.maxHumidityPct !== null;
    if (!dryConfigured)
        return noneMode("Entfeuchten nicht konfiguriert.");
    ...
    if (hum >= maxH) {
        const hours = capWithLearnedHours(input.remainingHours * DRY_IMMEDIATE_REMAINING_FRACTION, input.remainingHours, input.learnedHours != null ? input.learnedHours * 0.5 : null);
...
function predictiveCooling(input, fromMs, toMs) {
    ...
    return modeDemand(hours, `Predictive Kühlbedarf: Peak ${sim.peak.toFixed(1)} °C, Grenze ${input.unit.onTempC} °C`, {
        predictedPeak: round2(sim.peak),
        predictedLow: round2(sim.low),
        predictedCrossingAtIso: sim.crossingAtMs != null ? new Date(sim.crossingAtMs).toISOString() : input.now.toISOString(),
    });
}
function predictiveHeating(input, fromMs, toMs) {
    if (!(0, config_1.acModeCommandEnabled)(input.unit.modeWhenHeating)) {
        return noneMode("Climate Heating nicht verfügbar.");
    }
    const setC = input.unit.heatSetpointC;
...
function predictiveDry(input) {
    ...
    if (!thermal || hum == null)
        return null;
    if (!statUsable(thermal.dehumidify.humidity) || thermal.dehumidify.humidity.rate == null)
        return null;
...
function estimateClimateUnitDemand(input) {
    const modes = (0, config_1.availableAcModePurposes)(input.unit);
    const remaining = input.remainingHours;
    const empty = (model, reason) => ({
        demandModel: model,
        fallbackReasonDe: reason,
        predictiveConfidence: null,
        cooling: noneMode(reason),
        heating: noneMode((0, config_1.acModeCommandEnabled)(input.unit.modeWhenHeating) ? reason : "Climate Heating nicht verfügbar."),
    ...
    const coolingEnabled = (0, config_1.acModeCommandEnabled)(input.unit.modeWhenCooling);
    const heatingEnabled = (0, config_1.acModeCommandEnabled)(input.unit.modeWhenHeating);
    const dryEnabled = (0, config_1.acModeCommandEnabled)(input.unit.modeWhenDehumidify) && input.unit.maxHumidityPct !== null;
    ...
    if (missingRoomAndNeedTemp && !canPredictDry) {
        ...
        const heating = noneMode(heatingEnabled ? "Raumtemperatur fehlt — kein Heating ohne Sensor." : "Climate Heating nicht verfügbar.");
    ...
    let heating;
    let dehumidify;
    let demandModel = "bootstrap";
    let fallbackReasonDe = null;
    let predictiveConfidence = null;
...
exports.estimateClimateUnitDemand = estimateClimateUnitDemand;
function climateWindowEndMs(now, remainingHours) {
    return now.getTime() + Math.max(0, remainingHours) * 3_600_000;
}
exports.climateWindowEndMs = climateWindowEndMs;
function climateDemandDigest(input) {
    return input.units
        .map((u) => {
...
exports.climateDemandDigest = climateDemandDigest;
