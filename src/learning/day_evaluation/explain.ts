...
export type DeterministicDayExplanation = {
	...
	heizstab: {
		windows: Array<{ startIso: string; endIso: string; energyKwh: number; powerW: number }>;
		totalKwh: number;
		reasonCodes: string[];
	};
	klima: {
		windows: Array<{ startIso: string; endIso: string; energyKwh: number; unitId: string }>;
		totalKwh: number;
		reasonCodes: string[];
	};
	fahrzeug: {
		...
		plannedPvKwh: number | null;
		plannedGridKwh: number | null;
	...
	risiken: string[];
	goals: Array<{ consumerId: string; goalId: string; met: boolean | null; detailDe: string }>;
	reasonCodes: string[];
};
...
export function buildDeterministicDayExplanation(
	...
	if (!eco?.exportTariffKnown) risks.push("export_tariff_unknown");
