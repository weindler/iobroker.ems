...
async function fetchHistoryRowsAggregated(host, stateId, startMs, endMs, count, timeoutMs, aggregate, stepMs) {
    ...
    for (let i = 0; i < candidates.length; i++) {
        ...
        const { rows, timedOut, error } = await invokeGetHistory(host, candidateId, {
        ...
        if (normalized.length > 0) {
            if (i > 0 && host.log?.warn) {
                host.log.warn(`History aggregate: Daten über Fallback-State ${candidateId} (${normalized.length} Zeilen, konfiguriert: ${stateId})`);
            }
            return normalized;
        }
    }
...
async function fetchHistoryRowsLookbackForId(host, stateId, lookbackDays, countPerDay, timeoutMs) {
    ...
    if (host.log?.debug) {
        host.log.debug?.(`History query: Tages-Fallback ${lookbackDays}d für ${stateId} (${formatHistoryStats(bulk.stats)})`);
    }
