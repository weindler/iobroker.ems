export type CoverageStatus = "covered" | "intentionally_removed" | "unresolved";

export interface PlannerInputCoverageEntry {
	legacySource: "state" | "foreign_state" | "config" | "file";
	legacyId: string;
	consumer: string;
...
function entry(
	legacySource: PlannerInputCoverageEntry["legacySource"],
	legacyId: string,
	consumer: string,
	snapshotField: string,
	transform: string,
...
export const PLANNER_INPUT_COVERAGE_MATRIX: readonly PlannerInputCoverageEntry[] = [
	...
	entry(
		...
		"runPlannerTick",
		"—",
		"planner output not planner input",
		"intentionally_removed",
		"Prior tick output; worker recomputes intent",
	),
	entry(
		"state",
		"planner.intent.thermal.commanded_stage",
		"immersion runtime engine",
		"—",
		"feedback from prior planner write",
		"intentionally_removed",
		"Not consumed by listed planner ticks",
	),
	entry(
		"state",
		"planner.intent.thermal.target_temp_c",
		"immersion runtime engine",
		"—",
		"feedback from prior planner write",
		"intentionally_removed",
	...
	entry(
		"state",
		"forecast_plan.plan_json",
		"runDailyPlanTick",
		"—",
		"in-memory ForecastPlan passed between ticks",
		"intentionally_removed",
		"Worker receives forecast via job pipeline not state re-read",
	),
	entry(
		"state",
	...
	entry(
		...
		"learning",
		"learning.thermalRuntime.history",
		"from persist file not raw state blob",
		"intentionally_removed",
		"Avoid large JSON state hotpath",
	),
	entry(
		"state",
		"learning.thermal_runtime.by_season_json",
		"learning",
		"learning.thermalRuntime.bySeason",
		"from persist file",
		"intentionally_removed",
	),
	entry(
		"state",
		"learning.thermal_runtime.by_day_type_json",
		"learning",
		"learning.thermalRuntime.byDayType",
		"from persist file",
		"intentionally_removed",
	),
	entry(
	...
	entry(
		"config",
		"immersion_heater.stages[].setStateId",
		"runPlannerTick",
		"—",
		"device write path",
		"intentionally_removed",
		"Only stage label/power in thermal.config.stages",
	),
	entry(
		"config",
		"immersion_heater.stages[].feedbackStateId",
		"runPlannerTick",
		"—",
		"device read mapping path",
		"intentionally_removed",
	),
	entry(
		"state",
		"learning.thermal_runtime.last_run",
		"learning",
		"—",
		"not used by planner ticks",
		"intentionally_removed",
		"Diagnostic only",
	),
	entry(
		"state",
		"learning.thermal_runtime.last_error",
		"learning",
		"—",
		"not used by planner ticks",
		"intentionally_removed",
...
function BAT_ID(suffix: string): string {
...
export function coverageCounts(): Record<CoverageStatus, number> {
	...
	for (const row of PLANNER_INPUT_COVERAGE_MATRIX) {
		counts[row.status] += 1;
	}
	return counts;
}

export function assertCoverageMatrixComplete(): void {
	const unresolved = PLANNER_INPUT_COVERAGE_MATRIX.filter((r) => r.status === "unresolved");
	if (unresolved.length > 0) {
		throw new Error(`coverage matrix has ${unresolved.length} unresolved entries`);
	}
}

export const PLANNER_SNAPSHOT_FORBIDDEN_STATIC_IMPORTS = [
	"planner_worker/main",
	"planner/run",
	"planner/index",
