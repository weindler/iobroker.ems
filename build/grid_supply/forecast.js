...
function priceLearningFallbackCtPerKwh(input) {
    if (input.priceLearningStatus !== "ready")
        return null;
    const eur = input.priceLearningAvgPrice7dEur ?? input.priceLearningAvgPrice30dEur ?? input.priceLearningAvgPrice90dEur;
    if (eur === null || eur === undefined || !Number.isFinite(eur) || eur < 0)
        return null;
 ...
 */
function buildPriceLearningFallbackSlots(now, priceCtPerKwh, flexibleImportAllowed, effectiveMaxImportW) {
    const slots = [];
    ...
    for (let i = 0; i < slotCount; i++) {
        ...
        slots.push({
            startIso: (0, time_1.isoFromMs)(slotStartMs),
            endIso: (0, time_1.isoFromMs)((0, time_1.slotEndMsFromStart)(slotStartMs)),
            priceCtPerKwh,
            importAllowed: flexibleImportAllowed,
            maxImportPowerW: effectiveMaxImportW,
            priceLabel: "normal",
            quality: (0, quality_1.gridDataQuality)("degraded", "Price-Learning-Fallback (gelernter Ø-Preis, keine Stunden-Kurve)"),
        });
    }
    return slots;
}
...
function normalizeDynamicSlots(rawSlots, flexibleImportAllowed, effectiveMaxImportW, referenceMedianCt) {
    ...
    for (const raw of rawSlots) {
        ...
        const slot = {
            ...
            quality: (0, quality_1.gridDataQuality)(priceCtPerKwh === null ? "missing" : "valid", priceCtPerKwh === null ? "Preis für Slot unbekannt" : "Dynamischer Tarif-Slot"),
        };
...
function resolveSourceAndCurrentPrice(input) {
    if (input.dynamicSlots.length > 0) {
        ...
        return {
            ...
            quality: (0, quality_1.gridDataQuality)("valid", "Dynamischer Tarif mit Preis-Slots verfügbar"),
    ...
    if (learnedFallback !== null) {
        return {
            ...
            quality: (0, quality_1.gridDataQuality)("degraded", "Price-Learning-Fallback — keine aktuellen Tarif-Slots, gelernter Ø-Preis genutzt", null),
    ...
    if (input.fixedPriceCtPerKwh !== null && Number.isFinite(input.fixedPriceCtPerKwh) && input.fixedPriceCtPerKwh >= 0) {
        return {
            ...
            quality: (0, quality_1.gridDataQuality)("degraded", "Festpreis-Fallback — keine dynamischen Slots"),
    ...
    return {
        source: "none",
        currentPriceCtPerKwh: current,
        quality: (0, quality_1.gridDataQuality)("missing", "Keine Preisquelle verfügbar"),
...
function buildGridSupplyForecast(input) {
    ...
    const fallbackSlots = source === "price_learning_fallback" && currentPriceCtPerKwh !== null
    ...
    if (!flexibleImportAllowed) {
        quality = (0, quality_1.gridDataQuality)("disabled", reasonDe, quality.confidencePct);
    }
    else if (slots.length === 0 && source === "none") {
        quality = (0, quality_1.gridDataQuality)("missing", reasonDe);
    }
    else if (slots.some((s) => s.priceCtPerKwh === null)) {
        quality = (0, quality_1.gridDataQuality)("degraded", reasonDe);
    }
    return {
        generatedAt,
        validUntil,
