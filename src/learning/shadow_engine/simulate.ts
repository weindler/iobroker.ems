...
import { SHADOW_ENGINE_MODEL_VERSION, SHADOW_MAX_MISSING_SLOT_FRACTION } from "./constants";
import { splitExogenousLoad } from "./exogenous_load";
import { notEvaluableStrategyResult, type ShadowRealResult, type ShadowStrategyResult } from "./types";
...
function firstNonNull(arr: Array<number | null>): number | null {
	for (const v of arr) {
		if (v !== null) return v;
	}
	return null;
 ...
 */
export function computeRealDayResult(
...
export interface ReferenceNoEmsBatteryParams {
	usableCapacityKwh: number | null;
	minSocPct: number | null;
	maxSocPct: number | null;
	maxChargeW: number | null;
	maxDischargeW: number | null;
	/** Realer SOC zum Tagesbeginn (bevorzugt letzter Wert des Vortags) — Simulationsstartpunkt. */
...
export function simulateReferenceNoEms(
	day: DayTelemetryDayRecord,
	params: ReferenceNoEmsBatteryParams,
	feedInCtPerKwh: number | null,
): ShadowStrategyResult {
	if (
		params.usableCapacityKwh === null ||
		!(params.usableCapacityKwh > 0) ||
		params.minSocPct === null ||
		params.maxSocPct === null ||
		params.startSocPct === null
	) {
		return notEvaluableStrategyResult("reference_no_ems", [
	...
	const sim = simulateGreedyBatterySelfConsumption({
	...
	for (let i = 0; i < day.slotCount; i++) {
		...
		if (bc !== null) batteryChargeKwh += bc;
		if (bd !== null) batteryDischargeKwh += bd;
	}

	const n = day.slotCount;
	const evaluable = n > 0 && sim.missingSlots <= n * SHADOW_MAX_MISSING_SLOT_FRACTION && haveImportCost;
 ...
 */
export function simulateEmsWithoutAi(
	real: ShadowRealResult,
	aiOverrideActiveForDay: boolean,
): ShadowStrategyResult {
	if (aiOverrideActiveForDay) {
		return notEvaluableStrategyResult("ems_without_ai", [
