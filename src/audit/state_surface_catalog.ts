...
export const STATE_SURFACE_FAMILIES: StateSurfaceFamily[] = [
	...
	},
	{
		...
		idPattern: "system.* | live.* | execution.safety.* | economics.* | operator.briefing_de",
		estimatedStaticCount: 36,
	...
	},
	{
		...
		dataTypes: ["string", "number", "boolean"],
		readWrite: "mostly_read",
		producer: "planner/ensure_states, planner/run",
		consumers: ["VIS legacy planner.intent.*", "operator"],
		persistenceNeed: "session",
		avgOrMaxSizeHint: "planner.intent.last_json large",
		publicFeature: true,
		targetClass: "F_compatibility_contract",
		migrationStrategy: "VIS depends on subset; keep contract until VIS redesign; move last_json to file.",
		largeJsonIds: ["planner.intent.last_json", "planner.intent.battery.winter.windows_json"],
	},
	{
	...
	{
		id: "vehicle_profiles",
		label: "Wallbox vehicle profiles",
		idPattern: "addons.wallbox.vehicles.<vehicleId>.*",
		estimatedStaticCount: 0,
		dynamicNote: "~64 states × configured wb_vehicle_profiles (0 if none)",
		dataTypes: ["string", "number", "boolean"],
		readWrite: "mixed",
		producer: "addons/wallbox/vehicles/ensure_states",
		consumers: ["wallbox vehicles runtime"],
		persistenceNeed: "durable_state",
		avgOrMaxSizeHint: "mostly scalars",
		publicFeature: true,
		targetClass: "A_core_user",
		migrationStrategy: "Ensure ONLY configured vehicles; never pre-create empty slots.",
	},
	{
		id: "battery",
