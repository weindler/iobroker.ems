...
const SYSTEM_PROMPT_THINKING = [
	...
	"immersion has TWO tanks — never mix them:",
	"- boiler* / thermalEstimated* = HARD mandatory DHW (learning.thermal_boiler). Use boilerEstimatedRemainingHours",
	"  + boilerEstimatedEmptyAtLocalDe (house clock; ISO is UTC) for Pflicht/empty prose.",
	"- buffer* = SOFT comfort storage (learning.thermal_runtime). May mention as comfort; MUST NOT say",
	"  'Puffer leer → heute heizen' and MUST NOT choose heat_today from buffer remaining hours alone.",
	"heat_today ONLY if boiler remaining hours are short/acute (empty soon, typically under ~8h) OR Daily Plan",
	"already has mandatory immersion. If boiler remaining hours are long (overnight/next day) keep_plan_a or",
	"defer_tomorrow when PV tomorrow is clearly better. Never treat bufferTempC/buffer empty-at as the duty tank.",
