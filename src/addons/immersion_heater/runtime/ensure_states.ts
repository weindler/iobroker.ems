...
export async function ensureImmersionRuntimeStates(host: StateHost): Promise<void> {
	...
	const defs: StateDef[] = [
		...
		numState(IMMERSION_RUNTIME_STATES.boilerTemperatureC, "Boiler-Temperatur °C"),
		numState(IMMERSION_RUNTIME_STATES.boilerMinTempC, "Boiler Mindesttemperatur °C"),
		strState(IMMERSION_RUNTIME_STATES.hygieneJson, "Hygiene Persist (JSON)", "{}"),
		strState(IMMERSION_RUNTIME_STATES.hygieneStatusDe, "Hygiene-Status (DE)", ""),
		strState(IMMERSION_RUNTIME_STATES.temperatureStatus, "Temperatur-Status", "missing"),
		numState(IMMERSION_RUNTIME_STATES.planningMaxTempC, "Puffer-Max / Heizstab-Grenze °C"),
		numState(IMMERSION_RUNTIME_STATES.planTargetTempC, "Plan-Tagesziel °C (autoritativ)"),
		...
		numState(IMMERSION_RUNTIME_STATES.minRuntimeRemainingSec, "Mindestlaufzeit Rest (s)", 0),
		numState(IMMERSION_RUNTIME_STATES.minPauseRemainingSec, "Mindestpause Rest (s)", 0),
		numState(IMMERSION_RUNTIME_STATES.configMinimumRuntimeSec, "Admin Mindestlaufzeit (s)"),
		numState(IMMERSION_RUNTIME_STATES.configMinimumPauseSec, "Admin Mindestpause (s)"),
