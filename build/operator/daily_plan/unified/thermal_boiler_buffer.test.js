...
(0, node_test_1.describe)("boiler/puffer invariants", () => {
    ...
    (0, node_test_1.it)("Overnight-Lücke: emptyAt nach Fensterende vor nextPv → Cover bis nextPv, Hard > 0", () => {
        ...
        strict_1.default.equal(r.coverUntilMs, NEXT_PV);
        strict_1.default.ok(r.mandatoryEnergyKwh > 0.2, `expected overnight hard bridge, hard=${r.mandatoryEnergyKwh} reason=${r.reasonDe}`);
        strict_1.default.ok(r.economicHeadroomKwh > 1, `soft must remain, soft=${r.economicHeadroomKwh}`);
    });
});
